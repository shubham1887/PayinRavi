...
<tr>
    ...
    <td>
        <select style="width:300px;height:30px;" class="text" id="ddlSchDesc" onChange="ChangeAmount()" placeholder="Select Scheme Name.Click on drop down" name="ddlSchDesc">
          ...
          <?php
            $str_query = "select a.*,b.businessname from tblgroup a left join tblusers b on a.user_id = b.user_id where a.groupfor = 'Distributor' and b.user_id > 0 order by businessname";
    		$resultScheme = $this->db->query($str_query);		
    		foreach($resultScheme->result() as $row)
    		{
    			echo "<option value='".$row->Id."'>".$row->group_name."[".$row->businessname."]</option>";
    		}
        ?>
          </select>
...
<tr>
	<td> 					
         ...
         <?php 				
         $str_query = "select a.Id,a.group_name,b.businessname from tblgroup a left join tblusers b on a.user_id = b.user_id where a.groupfor = 'Agent' and (b.user_id = ? or b.user_id = 1) order by b.businessname"; 						
         $resultScheme = $this->db->query($str_query,array($result_user->row(0)->user_id));		 						
         foreach($resultScheme->result() as $row) 						
         { 							
         echo "<option value='".$row->Id."'>".$row->group_name."[".$row->businessname."]</option>"; 						
         } 						
...
<table class="table table-borderless" style="color:#000000;font-weight:normal;font-family:sans-serif;font-size:14px;overflow:hidden" >
    <tr>  
        <td colspan="2">    
            <?php echo $this->Service_model->getAccessRights_checkboxHTMLTABLE($result_user->row(0)->user_id,$result_user->row(0)->usertype_name); ?>
        </td> 
    </tr> 
</table>
...
<div class="card-body col-6"><!--- start card-body-->

<table class="table table-borderless" style="color:#000000;font-weight:normal;font-family:sans-serif;font-size:14px;overflow:hidden" >
    <tr>

        <td align="right">    
           <h6>Current Downline Count </h6>
        </td> 
        <td>
          <h6 class="btn btn-primary btn-round"><?php echo $this->Insert_model->getChildCount($result_user->row(0)->user_id); ?></h6>
        </td>  
      </tr>
      <tr>
        ...
        <td>
          <input type="text" name="txtIdLimit" id="txtIdLimit" class="form-control-sm" value="<?php echo $result_user->row(0)->id_limit; ?>">
        </td>
