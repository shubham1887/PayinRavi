                ...
                <div class="col-sm-12 col-lg-12">
                    <div class="card shadow-base bd-0">
                        <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
                                            <h6 class="card-title tx-uppercase tx-12 mg-b-0">Distributor Edit</h6>
                        ...
                        <div class="card-body">
                                 ...
                                 <form method="post" action="<?php echo base_url()."SuperDealer/dist_edit?crypt=".$this->Common_methods->encrypt("MyData");?>" name="frmdistributer_form1" id="frmdistributer_form1" autocomplete="off">
<input type="hidden" name="hiduserid" value="<?php echo $result_user->row(0)->user_id; ?>">
<table class="table">
<tbody>
<tr>
<td><h5>Dist Name :</h5><input type="text" class="form-control-sm" placeholder="Enter Dist Name." id="txtDistname" name="txtDistname" value="<?php echo $result_user->row(0)->businessname; ?>"  maxlength="100" style="width:300px;"/>
</td>
<td><h5>Select Parent:</h5>
<select id="ddlparent" name="ddlparent" class="form-control-sm" style="width:300px;">
<option value="1">Select</option>
<?php
  $rlstparent = $this->db->query("select user_id,businessname from tblusers where usertype_name='MasterDealer' and host_id = ?",array($this->session->userdata("SdId")));
...
$str_query = "select * from tblgroup where groupfor = 'Distributor' and user_id = ?";
    $resultScheme = $this->db->query($str_query,array($this->session->userdata("SdId")));   
    foreach($resultScheme->result() as $row)
    {
      echo "<option value='".$row->Id."'>".$row->group_name."</option>";
    }
