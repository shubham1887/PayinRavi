            ...
            <div class="card">
                <div class="card-content">
                    <div class="card-body pd-xs-10">
                        ...
                        <form action="<?php echo base_url(); ?>MasterDealer_new/AddUser" method="post">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="editor-field">
                                            <select class="form-control" data-val="true" data-val-required="Scheme is required." id="Pattern" name="Pattern" required="">
                                                <option value="">Select</option>
                                                <?php
                                                    $check_CREATE_GROUP_rights = $this->db->query("select user_id from access_rights_alloted where user_id = ? and rights_id = (select Id from access_rights where rights_name = 'CREATE_GROUP')",array($this->session->userdata("MdId")));
                                                    if($check_CREATE_GROUP_rights->num_rows() == 1)
                                                    {
                                                        $rsltgroups = $this->db->query("select Id,group_name from tblgroup where user_id = ?",array($this->session->userdata("MdId")));
                                                    }
                                                    else
                                                    {
                                                        $rsltgroups = $this->db->query("select Id,group_name from tblgroup where user_id = 1 and Id = (select downline_scheme from tblusers where user_id = ?)",array($this->session->userdata("MdId")));
                                                    }
