...
<div id="loadingGIF"><img style="width:100px;" src="<?PHP echo base_url(); ?>Loading.gif" /></div>
    ...
    <!-- ########## END: RIGHT PANEL ########## ---><div class="br-mainpanel">
              ...
              <thead class="thead-colored thead-primary" >
												<tr><th>GroupName</th><th>charge_type</th><th>charge_value</th>
		            <th>DateTime</th>
				    <th></th>
	            </tr>
				</thead>
				<tbody><?php foreach($data->result() as $row)
						      { ?>
                  <tr>
                  	<td class="hidden-480"><?php echo $row->Name; ?>
                		        <input type="hidden" id="hidName<?php echo $row->Id;?>" value="<?php echo $row->Name; ?>"></td>
                    <td class="hidden-480">
					
												<?php if($row->charge_type == "SLAB") {?>
												<span style="display: none" id="charge_type_<?php echo $row->Id; ?>">
													<?php echo $row->charge_type; ?>
												</span>
												<a href="<?php echo base_url()."_Admin/Dmt_slabs?crypt1=".$this->Common_methods->encrypt($row->Name)."&crypt2=".$this->Common_methods->encrypt($row->Id); ?>" target="_blank">
													<?php echo $row->charge_type; ?>
												</a>
												<?php } else { ?>
                    ...
                    <td class="hidden-480"><?php echo $row->charge_value; ?>
                		        <input type="hidden" id="hidcharge_value<?php echo $row->Id;?>" value="<?php echo $row->charge_value; ?>"></td>
											...
											<td>
												<div class="hidden-sm hidden-sm btn-group">
													...
													function editform(id)
													{
														...
														document.getElementById("hidPrimaryId").value =  id;
								  document.getElementById("HIDACTION").value =  "UPDATE";document.getElementById("txtGroupName").value =  document.getElementById("hidName"+id).value;document.getElementById("txtcharge_type").value =  document.getElementById("hidcharge_type"+id).value;document.getElementById("txtcharge_value").value =  document.getElementById("hidcharge_value"+id).value;
													}
													</script></div>
            									</div>
        								</div>
											...
											<div class="modal-body">
												<div class="row">
													...
													<input type="hidden" id="hidPrimaryId" name="hidPrimaryId">
																...
																<div>
																	<select name="txtcharge_type" id="txtcharge_type" class="form-control">
																	<option value="0">Select</option><option value="PER">PER</option><option value="AMOUNT">AMOUNT</option><option value="SLAB">SLAB</option>
															...
															<div class="space-4"></div>
																        <input type="hidden" id="HIDACTION" name="HIDACTION" value="INSERT">
																        <div class="form-group">
																            <label for="form-field-select-3">charge_value</label>
																            <div>
																	            <input type="text" name="txtcharge_value" id="txtcharge_value" class="form-control">
																            </div>
															            </div>
															            <div class="space-4"></div>
														<?php echo form_close();?>
