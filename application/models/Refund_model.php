...
{	
	...
	public function recharge_check_refund_done($recharge_id)
	{
		...
		if($recharge_info->num_rows() == 1)
		{
		    ...
		    $total_credit = 0;
		    $total_debit = 0;
		    $tblewallet_entries = $this->db->query("select IFNULL(Sum(credit_amount),0) as total_credit,IFNULL(Sum(debit_amount),0) as total_debit from tblewallet where rechage_id = ? and user_id = ?",array($recharge_id,$user_id));
		    if($tblewallet_entries->num_rows() > 0)
		    {
		        foreach($tblewallet_entries as $ew)
		        {
		        ...
		        if($total_debit > 0)
		        {
		            ...
		            {
		                $resparray = array(
		                                    "message"=>"Refund Already Done For This Recharge",
		                                    "status"=>1
		                                   );
