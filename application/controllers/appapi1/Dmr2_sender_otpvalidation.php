				...
				if($userinfo->num_rows() == 1)
				{
					...
					if($status == '1')
					{
						if(ctype_digit($sendermobile))
						{
						  ...
						  //  echo $requestid_rslt->num_rows();exit;
					        ...
					        {
								$requestid_rslt = $this->db->query("select * from remitters where mobile = ? order by Id desc limit 1",array($sendermobile));
							
					          $requset_id = "";
					          $fname = $requestid_rslt->row(0)->name;
					          $lName = $requestid_rslt->row(0)->lastname;
					          $pincode = $requestid_rslt->row(0)->pincode;
					           $address1 = "ahmedabad";
        						$address2 = "gujarat";
