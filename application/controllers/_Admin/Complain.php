...
class Complain extends CI_Controller {
	...
	public function pageview()
	{
		...
		$result = $this->db->query("select complain_id from tblcomplain where user_id = ?",array($this->session->userdata("AgentId")));
		...
		$this->view_data['result_complain'] = $this->db->query("select * from tblcomplain where user_id = ? order by complain_id desc limit ?,?",array($this->session->userdata("AgentId"),$start_row,$per_page));
		$this->view_data['message'] =$this->msg;
		$this->view_data['cmpl_flag'] =0;
		$this->load->view('Retailer/complain_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
		...
		$this->pageview();			
			...
			else if($this->input->post("hidcomplain") == "Set")
			{
				$start_row = $this->uri->segment(3);
				$per_page = $this->common_value->getPerPage();
				if(trim($start_row) == ""){$start_row = 0;}
				...
				$this->view_data['message'] =$this->msg;
				$this->view_data['cmpl_flag'] =1;
				$this->view_data['recahrge_id'] =$this->input->post("recid");
				$this->load->view('Retailer/complain_view',$this->view_data);	
			}
			else
			{
