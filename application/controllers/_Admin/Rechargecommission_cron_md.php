...
class Rechargecommission_cron_md extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
		...
		$strrslt = $this->db->query("
		            SELECT 
		                ...
		                Sum(a.MDComm) as totalMDCommAmount,
		                md.businessname,
		                md.username,
		                md.usertype_name,
		                md.user_id as parentid
		                FROM `tblrecharge` a 
                        left join tblusers b on a.user_id = b.user_id
                        left join tblusers c on b.parentid = c.user_id
                        left join tblusers md on c.parentid = md.user_id
                        where 
                        Date(a.add_date) = ? and 
        ...
        foreach($strrslt->result() as $rw)
        {
            ...
            $md_commission = $rw->totalMDCommAmount;;
           ...
           // if(false)
    		...
    		else
    		{
    		    ...
    		    values(?,?,?,?,?,?,?,?,?)",
    		    array($this->common->getDate(),$this->common->getRealIpAddr(),$parentid,$transaction_date, $totalAmount,$md_commission,"RECHARGE","no",""));
    		    if($rsltinsert == true)
    		    {
    		        $insert_id = $this->db->insert_id();
    		        $cr_user_id = $parentid;
        		    $dr_user_id = 1;
        		    $amount = $md_commission;
