...
class Status_refund_to_success_report extends CI_Controller {
	...
	public function pageview()
	{
        ...
        $this->db->reconnect();
			...
			$rslt_sf = $this->db->query("SELECT 
										...
										IFNULL(Sum(a.commission_amount),0) as retComm,
										IFNULL(Sum(a.DComm),0) as DComm,
										IFNULL(Sum(a.MdComm),0) as MdComm,
										IFNULL(Sum(a.roffer),0) as roffer,
										IFNULL(Sum(a.AdminComm),0) as AdminComm,
										IFNULL(Sum(a.flat_commission),0) as flatcomm
										FROM statuschange_recharges b
										left join tblrecharge a on b.recharge_id = a.recharge_id
										where 
		...
		$this->load->view('_Admin/status_refund_to_success_view',$this->view_data);			
	}
...
$this->output->set_header("Pragma: no-cache"); 
	...
	public function dataexport()
	{
		...
		$rslt = $this->db->query($str_query,array($from,$to));
			...
			{
				$str_query = "select 
				sb.add_date as status_change_date,
		        a.debited,
		        a.reverted,
		        a.edit_date,
        		a.ewallet_id,
				...
				state.state_name,
				state.code as statecode
        		from statuschange_recharges sb
				left join tblrecharge a on a.recharge_id = sb.recharge_id
        		left join tblusers b on a.user_id = b.user_id 
				left join tblusers parent on b.parentid = parent.user_id
				left join tblcompany c on a.company_id = c.company_id 
				left join tblstate state on a.state_id = state.state_id
		...
		foreach($rslt->result() as $rw)
		{
			...
			$temparray = array(
									...
									"fail_to_success_date" =>$rw->status_change_date, 
    ...
    $fileName = "status_change_recharge_list From ".$from." To  ".$to.".xls";
...
write_file($filename, $sapretor."\n", 'a+');
	...
	private function gettablerowscount($from,$to,$api,$operator,$status,$numid,$start,$perpage,$user_id)
	{
		...
		$rsltrowcount=  $this->db->query("
				select 
				count(a.recharge_id) as total from statuschange_recharges b
				left join tblrecharge a on a.recharge_id = b.recharge_id
	    ...
	    if(isset($_POST["recharge_id"]) and isset($_POST["api_id"]))
	    {
	        ...
	        if($recharge_id > 0)
	        {
	            $recharge_info = $this->db->query("
                    		...
                    		left join tblusers c on a.user_id = c.user_id 
                    		where 
                    		a.recharge_id = ? and a.recharge_status = 'Pending'",array($recharge_id));
	            if($recharge_info->num_rows() == 1)
	            {
	                $company_id = $recharge_info->row(0)->company_id;
	                $Mobile = $recharge_info->row(0)->mobile_no;
