...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$this->session->set_userdata("txtRemitter",$txtRemitter);
				$this->session->set_userdata("txtAccNo",$txtAccNo);
				$this->session->set_userdata("txtUserId",$txtUserId);
				$this->session->set_userdata("txtAmount",$txtAmount);
				$this->session->set_userdata("ddlapi",$ddlapi);
				$this->session->set_userdata("ddldb",$ddldb);
				$this->session->set_userdata("dmt_setdata","yes");
				...
				$this->view_data['pagination'] = NULL;
				    ...
				    if($ddlstatus == "PENDING" or $ddlstatus == "HOLD")
    				{
    				    ...
    				    $this->view_data['result_all'] = $this->db->query("SELECT a.is_autohold,a.unique_id,
    				    ...
    				    a.ccf,a.cashback,a.tds,a.bank_id,
    ...
    left join bank_master bank on a.bank_id = bank.id
     ...
     order by Id desc limit 50",array($from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo,$txtUserId,$txtUserId,$txtAmount,$txtAmount,$ddlapi,$ddlapi));    
    				}
				...
				$this->view_data["summary_array"] = $this->getTotalDMRValues($from,$to,$ddlapi,$txtRemitter,$txtAccNo,$ddlstatus,$txtUserId);
...
//				print_r($this->input->post());exit;
				...
				if($hidaction == "STATUSUPDATE")
				{
					...
					SELECT a.is_autohold,a.unique_id,
    				    a.retry,a.API,a.Id,a.order_id,a.add_date,a.edit_date,a.user_id,a.Charge_type,a.charge_value,a.Charge_Amount,a.RemiterMobile,
    				    a.ccf,a.cashback,a.tds,a.bank_id,
    a.debit_amount,a.credit_amount,a.BeneficiaryId,a.AccountNumber,
    a.IFSC,a.Amount,a.Status,a.debited,a.balance,a.mode,
 ...
 where a.Id = ? order by Id desc",array($hidId));
							...
							if($paymentinfo->num_rows() >= 1)
							{
							   ...
							   // echo $difference;exit;
								if($difference == $Amount)
								{
									...
									$this->db->query("update mt3_transfer set Status = 'FAILURE',RESP_status = ' Refund ' where Id = ?",array($dmr_id));
			...
			else
			{
				...
				if(trim($user) == 'Admin' or trim($user) == 'EMP')
				{
				    ...
				    $ddldb = "LIVE";
				    $ddltype = "ALL";
				if($this->session->userdata("dmt_setdata") == "yes")
				{
				    $from = $this->session->userdata("from");
    				$to = $this->session->userdata("to");
    				$ddlstatus = $this->session->userdata("ddlstatus");
    				$txtRemitter = $this->session->userdata("txtRemitter");
    				$txtAccNo = $this->session->userdata("txtAccNo");
    				$txtUserId = $this->session->userdata("txtUserId");
    				$ddlapi = $this->session->userdata("ddlapi");
    				$ddldb = $this->session->userdata("ddldb");
    				$ddltype = $this->session->userdata("ddltype");
				}
...
if( ? != 'ALL',a.Status = ?,true) and
if(? != '',a.RemiterMobile = ?, true) and 
if(? != '',(a.AccountNumber = ? or a.RESP_opr_id like ?), true) and 
     
if( ? != 'ALL',a.API = ?,true) 
 order by Id desc limit 50",array($from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo,$txtAccNo,$ddlapi,$ddlapi));
	...
	public function getTotalDMRValues($from,$to,$ddlapi,$Remitter = "",$AccNo = "",$status = "ALL",$UserId = "")
	{
		$total_success = 0;
		$total_failure = 0;
		$total_pending = 0;
		...
		IFNULL(Sum(a.ccf),0) as total_ccf,
		IFNULL(Sum(a.cashback),0) as total_cashback,
		IFNULL(Sum(a.tds),0) as total_tds,
		...
		if(? != 'ALL',a.API =?,true)  and
		if( ? != 'ALL',a.Status = ?,true) and
	    if(? != '',a.RemiterMobile = ?, true) and 
	    if(? != '',a.AccountNumber = ?, true) and 
	    if(? != '',b.username = ?, true) 
		group by a.Status",array($from,$to,$ddlapi,$ddlapi,$status,$status,$Remitter,$Remitter,$AccNo,$AccNo,$UserId,$UserId));
	...
	public function checkstatus()
	{
		...
		{
			$Id = $_GET["id"];
			$result= $this->db->query("SELECT a.RESP_ipay_id,a.Id,a.order_id,a.add_date,a.edit_date,a.user_id,a.Charge_type,a.charge_value,a.Charge_Amount,a.RemiterMobile,
a.debit_amount,a.credit_amount,a.BeneficiaryId,a.AccountNumber,
a.IFSC,a.Amount,a.Status,a.debited, a.balance,a.mode,
...
FROM `mt3_transfer` a where a.Id = ? order by Id desc",array($Id));
			if($result->num_rows() == 1)
			{
			    ...
			    $RESP_ipay_id = $result->row(0)->RESP_ipay_id;
        		...
        		else
        		{
        		    ...
        		    else if($result->row(0)->API == "ZPULS")
        		    {

        		       $this->load->model("Zpuls_model");
				        echo $this->Zpuls_model->transfer_status_zpuls($Id);exit;             
        		    }
        		    else if($result->row(0)->API == "PAYTM")
        		    {

        		       $this->load->model("Paytm");
				        echo $this->Paytm->transfer_status($Id);exit;             
        		    }
        		    else if($result->row(0)->API == "Razorpay")
        		    {

        		       $this->load->model("RazorpayPayout_model");
				        echo $this->RazorpayPayout_model->payoutstatuscheck($RESP_ipay_id);exit;             
        		    }
