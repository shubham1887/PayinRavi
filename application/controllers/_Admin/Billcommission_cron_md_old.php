...
class Billcommission_cron_md_old extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
       ...
       // $transaction_date = '2019-03-21';
        ...
        a.bill_amount < 100000 and b.host_id != 4381 and
         m.user_id != 29 and
        m.user_id != 2408
    	...
    	foreach($rsltuser->result() as $rw)
    	{
		    ...
		    if($givencommission_result->num_rows() == 1)
		    {
		     ...
		     $transaction_date = "2019-08-19";
		    
		    $due_amount = $rw->total - $tltal_amount_commissiongiven ;
		    
		    if($host_id == 4381)
		    {
		        $commamount = (($due_amount * 0.30) / 100);    
		        $description = "BillComm.Date(OLD):".$transaction_date .",(".$due_amount." * 0.30 /100)";
		    }
		    else
		    {
		        $commamount = (($due_amount * 0.10) / 100);
		         $description = "BillComm.Date(OLD):".$transaction_date .",(".$due_amount." * 0.10 /100)";
		    }
