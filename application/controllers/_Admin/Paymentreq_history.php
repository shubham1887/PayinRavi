...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$this->view_data['result_mdealer'] = $this->db->query("select * from tblautopayreq where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? and host_id = 1",array($user_id,$from_date,$to_date));
				$this->view_data['flagopenclose'] =1;
				$this->view_data['message'] =$this->msg;
				$this->load->view('Retailer/paymentreq_history_view',$this->view_data);		
			}					
			...
			else
			{
				...
				$this->view_data['pagination'] = NULL;
				$this->view_data['result_mdealer'] = $this->db->query("select * from tblautopayreq where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? and host_id = 1",array($user_id,$today_date,$today_date));
				$this->view_data['flagopenclose'] =1;
				$this->view_data['message'] =$this->msg;
				$this->load->view('Retailer/paymentreq_history_view',$this->view_data);		
				}
