...
class Requestlog extends CI_Controller {
	...
	public function pageview()
	{
		...
		$config['base_url'] = base_url()."_Admin/requestlog/pageview";
		...
		select * from tblreqresp 
		where 
		Date(add_date) >= ? and 
		Date(add_date) <= ? and
		if(? != '',(request like ? or response like ?),true)
		order by Id desc limit ?,?",array($fromdate,$todate,$txtNumId,$like1,$like1,intval($start_row),intval($per_page)));	
		$this->view_data['message'] =$this->msg;
		$this->load->view('_Admin/requestlog_view',$this->view_data);			
	}
