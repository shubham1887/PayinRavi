...
$this->output->set_header("Pragma: no-cache"); 
		...
		if($this->input->post("btnSubmit")){
    	    ...
    	    $this->db->db_debug = TRUE;
				...
				{			
					var_dump($downline_scheme."     ".$downline_scheme2);
					$response = $this->Insert_model->tblusers_registration_Entry($parentid,$distributer_name,$postal_address,$pincode,$state_id,$city_id,$contact_person,$mobile_no,$emailid,$usertype_name,$status,$scheme_id,$username,$password,$aadhar,$pan_no,$gst,$downline_scheme,$downline_scheme2,$BDate,$service_array);
					...
					if(isset($jsonobj->message) and isset($jsonobj->status))
					{
						...
						if($status == "0")
						{
							...
							redirect("_Admin/api_user_registration");
						}
						else
						{
				...
				{	$reg_data = array(
					...
					$this->load->view('_Admin/api_user_registration_view',$data);
				}
