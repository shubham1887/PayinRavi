...
class Billcommission_cron_old extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
       ...
       // $transaction_date = '2019-03-21';
        ...
        Date(a.add_date) >= '2019-08-01' and
        a.status = 'Success'  and
        a.bill_amount < 100000 and b.host_id != 4381
        group by p.user_id order by a.Id",array($transaction_date));
    	foreach($rsltuser->result() as $rw)
    	{
		    ...
		    $givencommission_result = $this->db->query("SELECT IFNULL(Sum(TotalSuccess),0) as totalsuccess,IFNULL(Sum(totalcommission),0) as totalcommission FROM `bill_distcommission` where user_id = ? and Date(transaction_date) >= '2019-08-01'",
		    array($rw->user_id));
		    if($givencommission_result->num_rows() == 1)
		    {
		        $tltal_amount_commissiongiven = $givencommission_result->row(0)->totalsuccess;
		        $tltal_commissiongiven = $givencommission_result->row(0)->totalcommission;
		    }
		  ...
		  // // echo $cr_user_id."   >> ".$rw->total."   >> ".$commamount;
		    //echo "<br>";
		   // echo $cr_user_id."   >> ".$tltal_amount_commissiongiven."   >> ".$tltal_commissiongiven;
		  //  
		    
		    $due_amount = $rw->total - $tltal_amount_commissiongiven ;
		    
		    $commamount = (($due_amount * 0.20) / 100);
		    $tds = 0;
		    $commamount_aftertds = round($commamount - $tds,2);
		    $remark = "BILL PAYMENT COMM. (OLD) ";
		    $description = "Bill Payment Comm.(OLD): (".$due_amount." * 0.20 ) / 100,";
		    $payment_type = "CASH";
		    
		    $locking_date = "AUG1To20_19";
		    //echo $commamount_aftertds." ".$description ;exit;
		  if($this->checkduplicate($cr_user_id,$locking_date) == false)
		  //  if(false)
    		{
    		...
    		else
    		{
    		    ...
    		    $rsltinsert = $this->db->query("insert into bill_distcommission(add_date,ipaddress,user_id,transaction_date,TotalSuccess,totalcommission,tds,commission_type,commission_given,ewallet_id) 
                		    values(?,?,?,?,?,?,?,?,?,?)",
                		    array($this->common->getDate(),$this->common->getRealIpAddr(),$cr_user_id,$transaction_date, $due_amount,$commamount_aftertds,$tds,"BILL","no",""));
    		    if($rsltinsert == true)
    		    //if(true)
    		    {
        		    ...
        		    $remark = "BILL Payment Commission (OLD)";
        		    $description = "BillComm.Date(OLD):".$transaction_date .",(".$due_amount." * 0.20 /100)";
        		    $payment_type = "CASH";
