<?php
class DmtDailySale extends  CI_Controller 
{   
    function _construct()
    {
    ...
    public function index()
    {
        ...
        where (a.transaction_type = 'PAYMENT' or a.transaction_type = 'CRADIT') and Date(a.add_date) >= ? and Date(a.add_date) <= ?
...
///////////////accval cr dr
            $accval_cr_dr = $this->db->query("
                SELECT  
                 Sum(a.credit_amount) as total_credit_payment,
                Sum(a.debit_amount) as total_debit_payment,
                b.businessname,b.username,b.usertype_name,b.user_id
                FROM tblewallet2 a
                left join tblusers b on a.user_id = b.user_id
                where a.transaction_type = 'DMR' and a.description like 'Valid.Charge :%' and 
                Date(a.add_date) BETWEEN ? and ?
                group by a.user_id
                ",array($from,$to));
            foreach($accval_cr_dr->result() as $rwaccval)
            {
                $username_array[$rwaccval->user_id] = $rwaccval->username;
                $businessname_array[$rwaccval->user_id] = $rwaccval->businessname;
                $usertype_array[$rwaccval->user_id] = $rwaccval->usertype_name;
                array_push($user_array,$rwaccval->user_id);
                $rsltoptrdistgrpc[$rwaccval->user_id]["DMTACCVAL"]["CR"] = $rwaccval->total_credit_payment;
                $rsltoptrdistgrpc[$rwaccval->user_id]["DMTACCVAL"]["DR"] = $rwaccval->total_debit_payment;
            }



                 //           print_r($rsltoptrdistgrpc);exit;
            
            $rsltoptrdistgrp =  $this->db->query("select 
                            count(a.order_id) as totalcount, 
                            b.username,b.businessname,b.usertype_name,
                            a.user_id,
                            Sum(a.Amount) as Amount,
                             Sum(a.Charge_Amount) as Charge_Amount,
                            Sum(a.ccf) as ccf,
                              Sum(a.cashback) as cashback,
                              Sum(a.gst) as gst,
                        Sum(a.tds) as tds 
                            from mt3_transfer a 
                            left join tblusers b on a.user_id = b.user_id
                            ...
                            if($rsltoptrdistgrp != NULL)
                            {
                                if($rsltoptrdistgrp->num_rows() > 0)
                                {
                                    foreach($rsltoptrdistgrp->result() as $rw)
                                    {
                                        ...
                                        $rsltoptrdistgrpc[$rw->user_id]["DMT"]["Total"] = $rw->totalcount;
                                        $rsltoptrdistgrpc[$rw->user_id]["DMT"]["Amount"] =$rw->Amount;
                                        $rsltoptrdistgrpc[$rw->user_id]["DMT"]["Charge_Amount"] = $rw->Charge_Amount;
                                        $rsltoptrdistgrpc[$rw->user_id]["DMT"]["ccf"] = $rw->ccf;
                                        $rsltoptrdistgrpc[$rw->user_id]["DMT"]["gst"] = $rw->gst;
                                        $rsltoptrdistgrpc[$rw->user_id]["DMT"]["tds"] = $rw->tds;
                                    }
    ...
    a.usertype_name != 'Admin'");
                    ...
                    foreach($rsltuses->result() as $usr_row)
                    {
                        ...
                        if(isset($rsltoptrdistgrpc[$usr]["DMT"]["Total"]))
                        {
                            ...
                            $grossdmttotal =$rsltoptrdistgrpc[$usr]["DMT"]["Total"];
                            $grossdmtamount = $rsltoptrdistgrpc[$usr]["DMT"]["Amount"];
                          
                            $grossdmtccf = $rsltoptrdistgrpc[$usr]["DMT"]["ccf"];
                            $grossdmtgst = $rsltoptrdistgrpc[$usr]["DMT"]["gst"];
                            $grossdmttds =$rsltoptrdistgrpc[$usr]["DMT"]["tds"];
                        }
                        ...
                        {
                            $getopening = $this->db->query("select balance from tblewallet2 where user_id = ? and Date(add_date) < ? order by Id desc limit 1",array($usr,$from));
                        ...
                        $temparray["Dmt Charges"]  =$grossdmtcharge;
                        $temparray["dmtamount"]  =$grossdmtamount;

                        $grossaccval  = $accval_dr - $accval_cr;
                        $temparray["Dmt_AccountVerify"]  = $grossaccval;

                        $temparray["TotalRecharge"]  =$grossrecharge;
                         $temparray["AMOUNT"]  =$grossdmtamount;
                        $temparray["CCF"]  =$grossdmtccf;
                        $temparray["GST"]  =$grossdmtgst;
                        $temparray["TDS"]  =$grossdmttds;
                        ...
                        $closingbalance_by_calculation = ($openingbalance + $grosspurchase  + $grossrecharge_commission) - ($grosstransfer + $grossdmtcharge +  $grossdmtamount + $grossaccval);
        ...
        else
        {
            ...
            $this->load->view("_Admin/DmtDailySale_view",$this->view_data);
        }
