...
class Sd_list extends CI_Controller {
	...
	{ 
		...
		$userrows = $this->db->query("select count(user_id) as total from tblusers where usertype_name = 'SuperDealer'");
		
		$total_row = $userrows->row(0)->total;
		$this->load->library('pagination');
		$config['base_url'] = base_url()."_Admin/sd_list/pageview";
		...
		$this->load->view('_Admin/sd_list_view',$this->view_data);		
	...
	public function index() 
	{
		...
		else 		
		{
			...
			if($this->input->post("action") == "delete")
			{
			    ...
			    $userinfo = $this->db->query("select * from tblusers where user_id = ? and usertype_name = 'SuperDealer'",array($mdid));
			    ...
			    if($userinfo->num_rows() == 1)
			    {
			        ...
			        if($rsltdownline->num_rows()  >= 1)
			        {
			            ...
			            redirect(base_url()."_Admin/sd_list?crypt=".$this->Common_methods->encrypt("MyData"));
			        }
			        else
			        {
