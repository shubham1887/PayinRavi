...
class Dmr_report_hold extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$txtAmount = intval($this->input->post("txtAmount",TRUE));
				$ddlholdtype = $this->input->post("ddlholdtype",TRUE);
				
				$this->view_data['pagination'] = NULL;
				
    			$this->view_data['result_all'] = $this->db->query("SELECT a.is_autohold,bank.bank_name,a.unique_id,a.retry,a.API,a.Id,a.order_id,a.add_date,a.edit_date,
...
if(? != 'ALL',a.is_autohold = ?,true) 
order by Id desc",array($from,$to,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo,$txtUserId,$txtUserId,$txtAmount,$txtAmount,$ddlholdtype,$ddlholdtype));
				...
				$this->view_data['ddlholdtype'] =$ddlholdtype;
				
				$this->view_data["summary_array"] = $this->getTotalDMRValues($from,$to,$txtRemitter,$txtAccNo,$txtUserId,$txtAmount);
				$this->load->view('_Admin/dmr_report_hold_view',$this->view_data);		
			}
			else if(isset($_POST["hidactionall"]))
			{
				$hidactionall = $this->input->post("hidactionall");
				$hidactionallstatus = $this->input->post("hidactionallstatus");
				if($hidactionallstatus == "ResendZPLUS")
				{
					$API = 'ZPLUS';
					$chkdmr = $this->input->post("chkdmr");
					foreach($chkdmr as $dmr_id)
					{
						$this->load->model("Zpuls_model");
						$resp = $this->Zpuls_model->Resend_Api_call_only($dmr_id);
					}
				}
				else if($hidactionallstatus == "ResendPAYTM")
				{
					$API = '`PAYTM';
					$chkdmr = $this->input->post("chkdmr");
					foreach($chkdmr as $dmr_id)
					{
						...
						$resp = $this->Paytm->transfer_resend_hold_Paytm($dmr_id);
					}
				}
				else if($hidactionallstatus == "ResendAIRTEL")
				{
					$API = 'AIRTEL';
					$chkdmr = $this->input->post("chkdmr");
					foreach($chkdmr as $dmr_id)
					{
						...
						$resp = $this->Airtel_model->Resend_Api_call_only($dmr_id);
					}
				}
				...
				$this->session->set_flashdata("MESSAGEBOX","Action Submitted Successfully");
				redirect(base_url()."_Admin/dmr_report_hold");
...
//				print_r($this->input->post());exit;
				...
				if($hidaction == "STATUSUPDATE")
				{
					...
					SELECT 
					a.is_autohold,bank.bank_name,a.unique_id,a.retry,a.API,a.Id,a.order_id,a.add_date,a.edit_date,a.ccf,a.cashback,a.tds,
 ...
 where a.Id = ? order by Id desc",array($hidId));
					...
					if($rslttxncheck->num_rows() == 1)
					{
						...
						else if(($Status == "PENDING" or  $Status == "HOLD" or  $Status == "SUCCESS") and $hidstatus == "Failure")
						{
							...
							if($paymentinfo->num_rows() >= 1)
							{
							   ...
							   // echo $difference;exit;
								if($difference == $Amount)
								{
									...
									$this->session->set_flashdata("MESSAGEBOXTYPE","SUCCESS");
									$this->session->set_flashdata("MESSAGEBOX","Transaction Failed Successfully");
							...
							redirect(base_url()."_Admin/dmr_report_hold?crypt=".$this->Common_methods->encrypt("MyData"));
						}
						else if(($Status == "PENDING" or  $Status == "HOLD" or  $Status == "") and $hidstatus == "Success")
						{
							...
							$this->session->set_flashdata("MESSAGEBOXTYPE","SUCCESS");
							$this->session->set_flashdata("MESSAGEBOX","Transaction Updated To Success");
							redirect(base_url()."_Admin/dmr_report_hold?crypt=".$this->Common_methods->encrypt("MyData"));
						}
						...
						else if(($Status == "PENDING" or  $Status == "HOLD" or  $Status == "") and $hidstatus == "ResendAIRTEL")
						{
							...
							$this->session->set_flashdata("MESSAGEBOXTYPE","SUCCESS");
							$this->session->set_flashdata("MESSAGEBOX","Transaction Resent To Airtel Successfully");
							redirect(base_url()."_Admin/dmr_report_hold?crypt=".$this->Common_methods->encrypt("MyData"));
						}
						else if(($Status == "PENDING" or  $Status == "HOLD" or  $Status == "") and $hidstatus == "ResendPAYTM")
						{
							$this->load->model("Paytm");
							$resp = $this->Paytm->transfer_resend_hold_Paytm($dmr_id);
							$this->session->set_flashdata("MESSAGEBOXTYPE","SUCCESS");
							$this->session->set_flashdata("MESSAGEBOX","Transaction Resent To Paytm Successfully");
							redirect(base_url()."_Admin/dmr_report_hold?crypt=".$this->Common_methods->encrypt("MyData"));
						}
						else if(($Status == "PENDING" or  $Status == "HOLD" or  $Status == "") and $hidstatus == "ResendZPLUS")
						{
							$this->load->model("Zpuls_model");
							$resp = $this->Zpuls_model->Transfer_Api_call_only($dmr_id);
							$this->session->set_flashdata("MESSAGEBOXTYPE","SUCCESS");
							$this->session->set_flashdata("MESSAGEBOX","Transaction Resent To ZPLUS Successfully");
							redirect(base_url()."_Admin/dmr_report_hold?crypt=".$this->Common_methods->encrypt("MyData"));
						}
...
left join dezire_banklist bank on a.bank_id = bank.Id
 where 
a.API = 'HOLD' and
Date(a.add_date) >= ? and Date(a.add_date) <= ? order by Id desc",array($date,$date));
					...
					$this->view_data['Amount'] ="";
					$this->view_data['txtUserId'] ="";
					$this->view_data['ddlapi'] ="ALL";
					$this->view_data['ddlholdtype'] ="ALL";
	...
	public function getTotalDMRValues($from,$to,$txtRemitter,$txtAccNo,$txtUserId,$txtAmount)
	{
		$total_hold = 0;
		$total_hold_count = 0;
		...
		if(? != '',b.username = ?,true) and
		if(? != '',a.Amount = ?,true) 
		",array($from,$to,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo,$txtUserId,$txtUserId,$txtAmount,$txtAmount));
		
		if($rslt->num_rows() == 1)
		{
			$total_hold = $rslt->row(0)->total;
			$total_hold_count = $rslt->row(0)->totalcount;
		}
