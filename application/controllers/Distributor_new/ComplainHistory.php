...
redirect(base_url().'login?crypt='.$this->Common_methods->encrypt("MyData")); 
	...
	//	echo $start_row;exit;
		...
		$this->load->view('Distributor_new/ComplainHistory_view',$this->view_data);			
	}
...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			if(isset($_POST["Fromdate"]) and isset($_POST["Todate"]))
			{
    			...
    			$this->view_data['result_all'] = $this->db->query("select * from tblcomplain where user_id = ? and Date(complain_date) BETWEEN ? and ?",array($this->session->userdata("DistId"),$from,$to));
			...
			else
			{
				...
				if(trim($user) == 'Distributor')
				{
			    	...
			    	$this->view_data['result_all'] = $this->db->query("select * from tblcomplain where user_id = ? and Date(complain_date) = ?",array($this->session->userdata("DistId"),$today_date));
			    	//print_r($this->view_data['result_all']->num_rows());exit;
					$this->view_data['message'] =$this->msg;
