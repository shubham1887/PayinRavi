...
class AddBalance extends CI_Controller {
	...
	public function index() 
	{
		...
		if(isset($_POST["Select_UserName"]) and isset($_POST["cramount"]) and isset($_POST["crcomment"]) ) 	
		{
			...
			if($txtAmount > 0)
			{
				$myBalance = $this->Common_methods->getAgentBalance($this->session->userdata("DistId"));
				if($myBalance >= $txtAmount)
				{
					$this->load->model("Common_methods");
					$userinfo = $this->db->query("select user_id,businessname,username,usertype_name from tblusers where username = ? and parentid = ?",array($User_mobile,$this->session->userdata("DistId")));
					if($userinfo->num_rows() == 1)
					{
						...
						{
							$response = $this->Ew2->DealerAddBalance($this->session->userdata("DistId"),$cr_user_id,$txtAmount,$txtRemark);	
						}
						else
						{
						...
						if(isset($json_obj->status) and isset($json_obj->message) )
						{
							...
							if($status == "0")
							{
								...
								redirect(base_url()."Distributor_new/AddBalance?crypt=".$this->Common_methods->encrypt("MyData"));
							}
							else
							{
		...
		else
		{
			...
			$this->load->view("Distributor_new/AddBalance_view",$this->view_data);	
		}
	...
	public function FindUserDetails()
	{
		...
		if(isset($_POST["requestType"]))
		{
			...
			$parentid = $this->session->userdata("DistId");
