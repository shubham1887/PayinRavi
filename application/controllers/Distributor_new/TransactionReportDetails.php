		...
		{ 	
				...
				if(trim($user) == 'Distributor')
				{
					if(isset($_GET["Id"]))
					{
						...
						$dmr_result = $this->db->query("select * from mt3_transfer where Id = ? and user_id = ?",array($Id,$this->session->userdata("DistId")));
						$this->view_data["message"] = "";
						$this->view_data["dmr_result"] = $dmr_result;
						$this->load->view("Distributor_new/TransactionReportDetails_view",$this->view_data);
					}		
				}
				else
				{redirect(base_url().'login');}																								
