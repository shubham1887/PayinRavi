...
class Statement extends CI_Controller {
	...
	//	echo $start_row;exit;
		...
		$this->load->view('Distributor_new/Statement_view',$this->view_data);			
	}
...
*/
		$str_query ="
				...
				if($result->num_rows() > 0)
				{
					$Credit = 0;
					$Debit = 0;
					$summary_rslt = $this->db->query("select IFNULL(Sum(credit_amount),0) as Credit,IFNULL(Sum(debit_amount),0) as Debit from tblewallet where transaction_type = 'PAYMENT' and user_id = ? and Date(add_date) BETWEEN ? and ? order by Id",array($this->session->userdata("DistId"),$from,$to));
