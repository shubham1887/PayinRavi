...
class Razorpay_payout_callback extends CI_Controller {
	...
	public function logentry($data)
	{
		...
		$filename = "inlogs/razorpay_payout_callback.txt";
...
write_file($filename, $sapretor."\n", 'a+');
	...
	public function index2()
	{
		...
		//secret= eTqjZ6CtoCuiC9aQXulcCgkKFIpj0JCI
...
*/
	...
	public function index()
	{
		...
		$php_response = file_get_contents('php://input');
 		...
 		/*  $php_response = '{"entity":"event","account_id":"acc_FvQQcgb9k1IBAB","event":"payout.processed","contains":["payout"],"payload":{"payout":{"entity":{"id":"pout_JOmctMURPG2irO","entity":"payout","fund_account_id":"fa_JOmctBimzaM0BM","amount":2499900,"currency":"INR","notes":{"notes_key_1":"Beam me up Scotty","notes_key_2":"Engage"},"fees":0,"tax":0,"status":"processed","purpose":"refund","utr":"211818092390","mode":"IMPS","reference_id":"45300","narration":"Acme Corp Fund Transfer","batch_id":null,"failure_reason":null,"created_at":1651150453,"queueing_details":{"reason":null,"description":null},"status_details":{"reason":"payout_processed","description":"Payout is processed and the money has been credited into the beneficiaries account.","source":"beneficiary_bank"}}}},"created_at":1651150469}';
...
*/
		...
		if(isset($json_obj->entity))	
		{
			...
			if(isset($json_obj->payload))
			{
				$payout = $json_obj->payload->payout;
				...
				if(isset($entity->id))
				{
						...
						//echo $status."#".$utr."#".$reference_id;exit;
						$dmr_info = $this->db->query("select * from mt3_transfer where Id = ?",array($reference_id));
						if($dmr_info->num_rows() == 1)
						{
							$dmr_id = $dmr_info->row(0)->Id;
							//echo $dmr_id;exit;
							$db_Status = $dmr_info->row(0)->Status;
							$API = $dmr_info->row(0)->API;
							//echo $db_Status;exit;
							if($db_Status == "PENDING" and $API == "Razorpay" and $status == "processed")
							{
								$bank_ref_num = $utr;
								$data = array(
				    					'RESP_statuscode' => "TXN",
