...
class DoBillrecharge extends CI_Controller 
{
	...
	{ 
		...
		{
			if(isset($_GET['userid']) && isset($_GET['pin'])   && isset($_GET['mcode']) && isset($_GET['serviceno']) && isset($_GET['customer_mobile']) && isset($_GET['option1']) && isset($_GET['Amount']))
			{
						...
						if($company_info->num_rows() == 1)
						{
							...
							if($balance >= $Amount)
							{
								...
								$particulars = "";
					             ...
					             $resp = $this->BillPayment->recharge_transaction2($user_info,$company_info->row(0)->company_id,$Amount,$serviceno,$customer_mobile,$remark,$option1,$ref_id,$particulars,$option2,$option3,$done_by,$payment_mode,$order_id);
