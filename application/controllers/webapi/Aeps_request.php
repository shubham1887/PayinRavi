...
class Aeps_request extends CI_Controller 
{
...
//print_r($json_obj);exit;
   // echo $json_obj->username."|".$json_obj->password."|".$json_obj->apitoken."|".$json_obj->request;exit;
        ...
        if(isset($json_obj->username) && isset($json_obj->password) && isset($json_obj->apitoken) && isset($json_obj->request) )
        {
            ...
            $user_agent = $json_obj->user_agent;
            $guid = 'GUID'.date('YmdHis');

            $outlet_id = $json_obj->request->outlet_id;//120000
            $amount = $json_obj->request->amount;//100
            $aadhaar_uid = $json_obj->request->aadhaar_uid;//988449036446
            $bankiin = $json_obj->request->bankiin;//508534
            $latitude = $json_obj->request->latitude;//19.3219898
            $longitude = $json_obj->request->longitude;//84.798524
            $cmobile = $json_obj->request->mobile;//7077023417
            $agent_id = $json_obj->request->agent_id;//GUID20210928122811
            $sp_key = $json_obj->request->sp_key;//WAP

            $pidData = $json_obj->request->pidData;
           
            $pidDataType = $json_obj->request->pidDataType;//X
            $ci = $json_obj->request->ci;//20221021
            $dc = $json_obj->request->dc;//a0cc1672-d83d-43b9-bce7-2a63dfd5c5b3
            $dpId = $json_obj->request->dpId;//MANTRA.MSIPL
            $errCode = $json_obj->request->errCode;//0
            $errInfo = $json_obj->request->errInfo;//Capture+Success
            $fCount = $json_obj->request->fCount;//1
            $tType = $json_obj->request->tType;//0
            $hmac = $json_obj->request->hmac;//n%2FGE591zgvhTmS%2FY1qzX1vlyfjLrhN8EHcHuN03cK6F84yaV15bu8snDjIO0HCs8
            $iCount = $json_obj->request->iCount;//0


            $mc = $json_obj->request->mc;
            /*
              %2BYFJWNa0FJTT8PsZTgKA8VXj2ulISbKVczz2vZKk4%2FdholHiYqmGQFn25zk2dgky3QnXu8ZLBAoMAd9GuCAE%2Fije5Bm5U06%2BLwoT2FJTP2B3wepAwMsSqZkwSK2glTkP7jaPun0FG4PBJ87S%2BOBlc1Ll%2FQKYP0ufEHq4PQ710JdzGPMVvnBW1JKVu1DguUN3fHp2MTCmQLbqyj74mYZeW01JqaXidlB5j%2BhsmvS5Dj4t2U60PbhauWchbQVUvoKWh1a7daYt%2Bwrxg8DccSUatzit8KTpFvvXWN7xux1i4Qc330VOj0CAwEAATANBgkqhkiG9w0BAQsFAAOCAQEAANeoH9fF4OR1jLkZvo5wY871mpmQXmq%2FZunNgnbbw8Ya%2B40EXmfeQ6b7nGqbCOulczw1kLRpBS6xm%2BsLY%2BKi%2B9PBsd7nzhI59dW5fgtN3seOqSz5PaOCyqYHeD%2BZQNEkDEdmRXKgjU4R96T%2BR8DS7tVKp8KufQQHGcBN%2B3ZFOu03%2BlKTv6rR6LrYFOrt84OuU5loZ%2F%2B6y75CaDCOMUtp8%2FEu6SxnSo62hkje6YGlCy%2FHEQje6SyARHA8xhV2L8lV7KvHf3eKd%2BX4QSh3J0TBe8As3CxOLCGZzdNCpUKL%2FVgf0RW51xKCwC26qV95T%2Blkabks%2FsnPX3s%2BhaHsf%2BC9gw%3D%3D
            */
            $mi = $json_obj->request->mi;//MFS100
            $nmPoints = $json_obj->request->nmPoints;//16
            $pCount = $json_obj->request->pCount;//0
            $pType = $json_obj->request->pType;//0
            $qScore = $json_obj->request->qScore;//90
            $rdsId = $json_obj->request->rdsId;//MANTRA.AND.001
            $rdsVer = $json_obj->request->rdsVer;//1.0.5
            $sessionKey = $json_obj->request->sessionKey;
            /*
            JbaWrnxsRmkWejLplEh5dmHzWAQuYTfJZRqmFo%2F81UHTAE3ynPMlzSgBPs1%2BZ7f0db%2FNbDQyoXexOOgcU8%2FdoKyhdOwoJhANraStY%2FXdM0JjIss3fGsXTf5ugqKA7RCEWKX4AqSRIKHebkw41xRo41LkUKL1uiLVEPCSZyrduGa57zqlAqvGQLhAAnc1TUNa5YN%2BETRjeDqD2r0zKu2s1b9rUymKBO36qAawAjjLUlB2LHQdc5XxeC9Cy9SGq1wDgH9kK0eb4uPWblmzABZBKp15LSpjoopz4hJ6yANGU2Zk3vu13eYyr4IPeWgBtFnrXwCigBXyU9tN5obs%2BX60ng%3D%3D
            */           
            $srno = $json_obj->request->srno;//0357836
...
$rslt_insert = $this->db->query("insert into aeps_request(token, outlet_id, amount, aadhaar_uid, bankiin, latitude, longitude, mobile, agent_id, sp_key, pidData, pidDataType, ci, dc, dpId, errCode, errInfo, fCount, tType, hmac, iCount, mc, mi, nmPoints, pCount, pType, qScore, rdsId, rdsVer, sessionKey, srno, user_agent, add_date, ipaddress, user_id, usertoken_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array(
    $apitoken, $outlet_id, $amount, $aadhaar_uid, $bankiin, $latitude, $longitude, $cmobile, $guid, $sp_key, $pidData, $pidDataType, $ci, $dc, $dpId, $errCode, $errInfo, $fCount, $tType, $hmac, $iCount, $mc, $mi, $nmPoints, $pCount, $pType, $qScore, $rdsId, $rdsVer, $sessionKey, $srno, $user_agent, $add_date, $ipaddress, $user_id, $usertoken_id
  ));
if($rslt_insert == true)
{
