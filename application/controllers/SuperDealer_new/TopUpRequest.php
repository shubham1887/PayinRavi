		...
		{ 	
					...
					if($RequestUserID == 1 or $RequestUserID == $this->session->userdata("DistParentId"))
					{
						...
						if($Amount > 0)
						{
									...
									if($insert_data == true)
									{
										...
										redirect(base_url()."SuperDealer_new/TopUpRequest");
									}
									else
									{
				...
				else
				{
					$userinfo = $this->db->query("
								SELECT businessname,user_id,Date(add_date) as add_date,mobile_no,status,kyc FROM tblusers where user_id = ?",array($this->session->userdata("SdId")));
					//print_r($mycomm->result());exit;
					$this->view_data['myinfo'] = $userinfo;
					$this->view_data['message'] =$this->msg;
			
			
					$this->load->view('SuperDealer_new/TopUpRequest_view',$this->view_data);	
				}
		}
