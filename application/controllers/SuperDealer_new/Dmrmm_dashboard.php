...
redirect(base_url().'login?crypt='.$this->Common_methods->encrypt("MyData")); 
	...
	public function index() 
	{
		...
		/*$userinfo = $this->db->query("select mt_accept,mt_transo from tblusers where user_id = ?",array($this->session->userdata("SdId")));
		...
		if($this->input->post("txtMobileNO"))
		{
				...
				if(isset($jsonobj->message) and isset($jsonobj->status) and isset($jsonobj->statuscode))
				{
					...
					else if($status == "0")
					{
							...
							$this->load->view("SuperDealer_new/dmrmm_dashboard_view",$this->view_data);
                    ...
                    else
                    {
    					...
    					if(isset($jsonobj->message) and isset($jsonobj->status) and isset($jsonobj->statuscode))
    					{
    						...
    						else if($status == "0")
    						{
    								...
    								$this->view_data["data"] = $data;
    								$this->load->view("SuperDealer_new/dmr2_dashboard_view",$this->view_data);
