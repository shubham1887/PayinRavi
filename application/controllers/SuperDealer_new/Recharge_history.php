...
redirect(base_url().'login?crypt='.$this->Common_methods->encrypt("MyData")); 
	...
	//echo "asdfsdf";exit;
		...
		$user = $this->session->userdata("SdId");
		...
		else
		{
            ...
            $rslt_sp = $this->db->query("SELECT 
    						...
    						a.add_date < ?  and a.user_id = ? and a.recharge_status = 'Pending'",array($from,$to,$date,$this->session->userdata("SdId")));
		...
		$this->view_data['result_all'] = $this->get_recharge($from,$to,$this->session->userdata("SdId"),$start_row,$per_page,$word,$ddldb,$ddlstatus,$ddloperator);
		$this->view_data['message'] =$this->msg;
		$this->load->view('SuperDealer_new/recharge_history_view',$this->view_data);			
...
*/
	...
	private function get_recharge($start_date,$end_date,$user_id,$startrow,$perpage,$word,$ddldb,$ddlstatus,$ddloperator)
	{
		...
		if($ddldb == "ARCHIVE")
		{
				...
				$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("SdId"),$ddlstatus,$ddlstatus,$ddloperator,$ddloperator,$word,$word,intval($startrow),intval($perpage)));
	...
	private function get_recharge_all($start_date,$end_date,$user_id,$ddldb)
	{
		...
		tblcompany.company_id=tblrecharge.company_id and Date(tblrecharge.add_date)>=? and Date(tblrecharge.add_date)<= ? and tblrecharge.user_id=?  order by tblrecharge.recharge_id";		
			$result = $this->db->query($str_query,array($start_date,$end_date,$this->session->userdata("SdId")));
			return $result;
		}
		else
		{
