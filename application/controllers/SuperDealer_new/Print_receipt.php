...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			if(isset($_GET["cruptid"]))
			{
				...
				left join tblusers c on a.user_id = c.user_id where a.recharge_id = ? and a.user_id = ?",array($criptid,$this->session->userdata("SdId")));
				if($recinfo->num_rows() == 1)
				{
					$this->view_data["data"] = $recinfo;
					$this->load->view("SuperDealer_new/print_receipt_view",$this->view_data);		
				}
			}
