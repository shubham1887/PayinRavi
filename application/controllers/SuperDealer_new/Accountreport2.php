...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$rsltdebit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ?",array($this->session->userdata("SdId"),$from_date,$to_date));
				...
				$this->load->view('SuperDealer_new/accountreport2_view',$this->view_data);		
			}					
			else
			{
				...
				$this->view_data['message'] =$this->msg;
				$rsltcredit = $this->db->query("select IFNULL(Sum(credit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("SdId"),$from_date,$to_date));
				$rsltdebit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("SdId"),$from_date,$to_date));
	...
	private function gettotalcredit($user_id,$from_date,$to_date)
	{
		...
		if($from_date <= $archivdate and $to_date <= $archivdate)
		{
			...
			$rsltcredit = $this->db->query("select IFNULL(Sum(credit_amount),0) as total from gofullrc_archiv.tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("SdId"),$from_date,$to_date));
	...
	private function gettotaldebit($user_id,$from_date,$to_date)
	{
		...
		if($from_date <= $archivdate and $to_date <= $archivdate)
		{
			...
			$rsltcredit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from gofullrc_archiv.tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("SdId"),$from_date,$to_date));
			return $rsltcredit->row(0)->total;	
		...
		{
			$rsltcredit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("SdId"),$from_date,$to_date));
			return $rsltcredit->row(0)->total;
		}
