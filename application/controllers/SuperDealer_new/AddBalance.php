...
class AddBalance extends CI_Controller {
	...
	public function index() 
	{
		...
		if(isset($_POST["Select_UserName"]) and isset($_POST["cramount"]) and isset($_POST["crcomment"]) ) 	
		{
			...
			if($txtAmount > 0)
			{
				$myBalance = $this->Common_methods->getAgentBalance($this->session->userdata("SdId"));
				if($myBalance >= $txtAmount)
				{
					$this->load->model("Common_methods");
					$userinfo = $this->db->query("select user_id,businessname,username,usertype_name from tblusers where username = ? and parentid = ?",array($User_mobile,$this->session->userdata("SdId")));
					if($userinfo->num_rows() == 1)
					{
						...
						if(isset($json_obj->status) and isset($json_obj->message) )
						{
							...
							if($status == "0")
							{
								...
								redirect(base_url()."SuperDealer_new/AddBalance?crypt=".$this->Common_methods->encrypt("MyData"));
							}
							else
							{
		...
		else
		{
			...
			$this->load->view("SuperDealer_new/AddBalance_view",$this->view_data);	
		}
