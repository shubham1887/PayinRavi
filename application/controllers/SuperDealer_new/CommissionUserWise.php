...
$this->output->set_header("Pragma: no-cache"); 
		...
		$data['message']='';	
			...
			else
			{
				...
				$data['message']='';
				$this->load->view('SuperDealer_new/CommissionUserWise_view',$data);
			}
		} 			
	}
	...
	public function GetUserTypeWiseScheme()
	{
		...
		$data_array = array();
		$group_info = $this->db->query("select Id,group_name from tblgroup where user_id = ? order by group_name",array($this->session->userdata("SdId")));
	...
	public function GetcommissiondetailsthroughPartial()
	{
		if(isset($_POST["schemetype"]))
		{
			...
			$parent_info = $this->db->query("select scheme_id from tblusers where user_id = ?",array($this->session->userdata("SdId")));
			if($parent_info->num_rows() == 1)
			{
				...
				$downline_group_info = $this->db->query("select Id,group_name from tblgroup where Id = ? and user_id = ?",array($schemetype,$this->session->userdata("SdId")));
		...
		if(isset($_POST["commID"]) and isset($_POST["commPer"]) and isset($_POST["commVal"]) and isset($_POST["chargePer"]) and isset($_POST["chargeVal"]) and isset($_POST["pattern"]))
		{
			...
			if($parent_info->num_rows() == 1)
			{
				...
				if($parent_commission_rslt->num_rows() == 1)
				{
					...
					$downline_group_info = $this->db->query("select Id,group_name from tblgroup where Id = ? and user_id = ?",array($pattern,$this->session->userdata("SdId")));
					if($downline_group_info->num_rows() == 1)
					{
