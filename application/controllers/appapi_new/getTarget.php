...
class GetTarget extends CI_Controller {
	...
	{ 
	    ...
	    $host_id = 1;
		$userinfo = $this->db->query("select user_id,businessname,username,status,mobile_no,usertype_name,password from tblusers where username = ? and password = ?",array($username,$pwd));
		if($userinfo->num_rows() == 1)
		{
			...
			if($status == '1')
			{
    				...
    				"data"=>array(
    					...
    					'DMT_TARGET'=>0,
    					'DMT_SALES'=>0,
    					'Wallet1'=>$this->Common_methods->getAgentBalance($user_id),
    					'Wallet2'=>$this->Ew2->getAgentBalance($user_id),
    					)
    				);
