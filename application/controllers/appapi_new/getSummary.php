			...
			if(isset($_GET['username']) && isset($_GET['pwd']))
			{
        		...
        		if($userinfo->num_rows() == 1)
        		{
        			...
        			if($status == '1')
        			{
    			       ...
    			       $rechargerslt = $this->db->query("select IFNULL(Sum(credit_amount),0) as totalcredit,IFNULL(Sum(debit_amount),0) as totaldebit from tblewallet where user_id = ? and transaction_type = 'PAYMENT' and Date(add_date) = ? ",array($user_id,$this->common->getMySqlDate()));
    			       if($rechargerslt->num_rows())
    			       {
    			           $totaldebit = $rechargerslt->row(0)->totaldebit;
