				...
				if($user_info->row(0)->usertype_name == "Agent" or $user_info->row(0)->usertype_name == "Distributor" or $user_info->row(0)->usertype_name == "MasterDealer")
				{
					...
					if($company_info->num_rows() == 1)
					{
					   ...
					   if($mcode == "UGE" or $mcode == "PGE" or $mcode == "DGE" or $mcode == "MGE")
					   {
					       ...
					       $url = 'http://www.manpay.in/appapi1/getPlan/getBill?mcode='.$mcode.'&serviceno='.$serviceno;
					       //echo $url;exit;
					   ...
					   else
					   {
    						...
    						$response = $this->Instapay->recharge_transaction_validate($user_info,$mcode,$company_info->row(0)->company_id,"",$serviceno,$customer_mobile,$option1);
