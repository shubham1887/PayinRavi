...
$this->output->set_header("Pragma: no-cache"); 
		...
		if($this->input->post("btnSubmit"))
		{
				...
				$validateparent = $this->db->query("select * from tblusers where user_id = ? and usertype_name = 'Distributor' and host_id = ?",array($parentid,$user_id));
				if($validateparent->num_rows() == 0)
				{
					$reg_data = array(
						...
						$this->load->view('SuperDealer/agent_registration_view',$data);
				}
				else
				{
					...
					{	
							...
							{ 
								...
								if($status == "0")
								{
									...
									redirect("SuperDealer/agent_registration");
								}
								else
								{
