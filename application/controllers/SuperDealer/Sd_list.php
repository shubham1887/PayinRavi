...
class Sd_list extends CI_Controller {
	...
	{ 
		...
		$userrows = $this->db->query("select count(user_id) as total from tblusers where usertype_name = 'SuperDealer' and parentid = ?",array($this->session->userdata("SdId")));
		
		$total_row = $userrows->row(0)->total;
		$this->load->library('pagination');
		$config['base_url'] = base_url()."SuperDealer/sd_list/pageview";
		...
		where 
		a.usertype_name = 'SuperDealer'  and parentid = ?
		order by a.username limit ?,?",array($this->session->userdata("SdId"),intval($start_row),intval($per_page)));
        $this->view_data['txtAGENTName'] ="";
		$this->view_data['txtAGENTId'] ="";
		$this->view_data['txtMOBILENo'] ="";
		$this->view_data['message'] =$this->msg;
		$this->load->view('SuperDealer/sd_list_view',$this->view_data);		
	...
	{
		$SearchWord = substr($SearchWord,0,20);
		if($SearchBy == "Mobile")
		{
		...
		left join tblgroup	g on a.scheme_id = g.Id
		where 
		a.usertype_name = 'SuperDealer' and
		a.mobile_no = '".$SearchWord."' and
		a.parentid = ?
		order by a.username ",array($this->session->userdata("SdId")));
		return $result;	
		}		
		if($SearchBy == "Agent")
		{
		...
		where 
		a.usertype_name = 'SuperDealer' and
		a.businessname like '".$SearchWord."%' and
		a.parentid = ?
		order by a.username",array($this->session->userdata("SdId")));
		return $result;	
		}		
		...
		where 
		a.usertype_name = 'SuperDealer' and
		a.username like '".$SearchWord."%' and
		a.parentid = ?
		order by a.username",array($this->session->userdata("SdId")));
		return $result;	
		}
		...
		{
			redirect(base_url()."SuperDealer/sd_list?crypt=".$this->Common_methods->encrypt("MyData"));
		}				
	}
}