...
class Sd_registration extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
		...
		if($this->input->post("btnSubmit"))
		{
		        ...
		        $DomainName = $this->input->post("txtDomainName",TRUE);	
				...
				{			
					...
					$response = $this->Insert_model->tblusers_registration_Entry($parentid,$distributer_name,$postal_address,$pincode,$state_id,$city_id,$contact_person,$mobile_no,$emailid,$usertype_name,$status,$scheme_id,$username,$password,$aadhar,$pan_no,$gst,$downline_scheme,$downline_scheme2,$txtBDate,$DomainName);
					...
					if(isset($jsonobj->message) and isset($jsonobj->status))
					{
						...
						if($status == "0")
						{
							...
							redirect("SuperDealer/sd_registration");
						}
						else
						{
				...
				{	$reg_data = array(
					...
					$this->load->view('SuperDealer/sd_registration_view',$data);
				}
