...
function getCommission()
{
		...
		if($userinfo->num_rows() == 1)
		{
			...
			if($parent_id == $this->session->userdata("SdId"))
			{
				...
				else
				{
					...
					<td >'.$this->Userinfo_methods->getCommissionInfo($row->company_id,$this->session->userdata("SdId")).'</td>
					<td >';
...
{
	$id = $_GET["id"];
	$rsltdealer = $this->db->query("select * from tblusers where parentid = ? and usertype_name = 'Distributor'",array($this->session->userdata("SdId")));
	...
	public function index() 
	{
			...
			if(isset($_GET["company_id"]) and isset($_GET["com"]) and isset($_GET["id"]))
			{
					...
					if($parent_id == $this->session->userdata("SdId") and $usertype_name == "Distributor")
					{
