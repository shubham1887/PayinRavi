...
class Dist_registration extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
		...
		if($this->input->post("btnSubmit"))
		{
				...
				$parentid =  $this->input->post("ddlparent");	
				...
				$status = 0;
				$txtBDate = "";
				$validateparent = $this->db->query("select * from tblusers where user_id = ? and usertype_name = 'MasterDealer' and host_id = ?",array($parentid,$user_id));
				if($validateparent->num_rows() == 0)
				{
					$reg_data = array(
						...
						$data['flag'] = 'Invalid Parent Selection';
						$data['regData'] = $reg_data;					
						$data['message'] = "Invalid Parent Selection";
						$data['MESSAGEBOXTYPE'] = "danger";
						$data['MESSAGEBOX'] =  "Invalid Parent Selection";
					
						$this->load->view('SuperDealer/dist_registration_view',$data);
				}
				else
				{
					...
					{	
							...
							{ 
								...
								if($status == "0")
								{
									...
									redirect("SuperDealer/dist_registration");
								}
								else
								{
