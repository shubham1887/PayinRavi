...
class Set_commission extends CI_Controller {
	...
	function getCommission()
	{
			...
			if($company_info->num_rows() == 1)
			{
				...
				$dist_commissioninfo = $this->Insert_model->getCommissionInfo($company_id,$this->session->userdata("SdId"),$this->session->userdata("MdSchemeId"));
	...
	function ChangeCommission()
	{
		...
		$userinfo = $this->db->query("select * from tblusers where user_id = ? and parentid = ?",array($user_id,$this->session->userdata("SdId")));
