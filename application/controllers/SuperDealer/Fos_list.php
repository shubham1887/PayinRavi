...
class Fos_list extends CI_Controller {
	...
	public function pageview()
	{
		...
		$userrows = $this->db->query("select count(user_id) as total from tblusers where usertype_name = 'FOS' and host_id = ?",array($this->session->userdata("SdId")));
		
		$total_row = $userrows->row(0)->total;
		$this->load->library('pagination');
		$config['base_url'] = base_url()."SuperDealer/fos_list/pageview";
		...
		a.host_id = ?
		order by a.businessname limit ?,?",array($this->session->userdata("SdId"),intval($start_row),intval($per_page)));
		$this->view_data['message'] =$this->msg;
		$this->view_data['txtAGENTName'] ="";
		$this->view_data['txtAGENTId'] ="";
		$this->view_data['txtMOBILENo'] ="";
		$this->load->view('SuperDealer/fos_list_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			{							
				...
				$userinfo = $this->db->query("select * from tblusers where user_id = ? and host_id = ?",array($user_id,$this->session->userdata("SdId")));
				if($userinfo->num_rows() == 1)
				{
					if($userinfo->row(0)->status == 0)
					{
						$password = $this->common->GetPassword();
						$this->db->query("update tblusers set status = 1,password = ? where user_id = ? and host_id = ?",array($password,$user_id,$this->session->userdata("SdId")));
						$this->load->model('Sms');
						...
						redirect(base_url()."SuperDealer/fos_list");
					}
					else if($userinfo->row(0)->status == 1)
					{
						//$password = $this->common->GetPassword();
						$this->db->query("update tblusers set status = 0 where user_id = ? and host_id = ?",array($user_id,$this->session->userdata("SdId")));
						//$this->load->model('Sms');
						//$this->Sms->passwordreset($userinfo->row(0)->username,$password,$userinfo->row(0)->mobile_no,$userinfo->row(0)->emailid,$userinfo->row(0)->businessname);
						$this->msg="Action Submit Successfully.";
						redirect(base_url()."SuperDealer/fos_list");
	...
	public function Search($SearchBy,$SearchWord)
	{
		...
		left join tblgroup	g on a.scheme_id = g.Id
    		where 
    		a.usertype_name = 'FOS' and b.host_id = ? and 
    		a.mobile_no = '".$SearchWord."' and 
    		order by a.username",array($host_id));
		    return $result;	
		}		
		if($SearchBy == "Agent")
		{
