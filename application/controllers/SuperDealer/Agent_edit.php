...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 
			...
			{				
				...
				$fosinfo = $this->db->query("select user_id,usertype_name,parentid from tblusers where user_id = ? and parentid = ? and usertype_name = 'FOS'",array($fos_id,$this->session->userdata("SdId")));
					...
					if($parent_info->row(0)->usertype_name == "Distributor")
					{
						...
						if($user_info->row(0)->mobile_no != $MobNo)
						{
							...
							if($mobrslt->num_rows() > 0)
							{
								...
								redirect(base_url()."SuperDealer/agent_edit/process/".$user_id);
								return;
							}
		                ...
		                $rsltupdate = $this->db->query("update tblusers 
									...
									",array($MobNo,$Dealername,intval($Parent_id),$State,$City,$MobNo,intval($Scheme_id),intval($User_id)));	
									if($rsltupdate == true)
									{
			...
			{
					$this->load->view('SuperDealer/agent_edit_view',$data);
			}
		} 			
	}
}
