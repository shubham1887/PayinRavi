...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	//print_r($this->input->post());exit;
			if($this->input->post('btnSearch') == "Search")
			{
        		...
        		$userinfo = $this->db->query("select * from tblusers where mobile_no = ? and host_id = ?",array($username,$host_id));
        		...
        		$this->load->view('SuperDealer/agent_account_report_view',$this->view_data);			
			}					
	...
	private function AccountLedger_getReport($user_id,$from_date,$to_date,$ddldb,$wallet,$transaction_type)
	{
	   ...
	   //echo $user_id."   ".$from_date."   ".$to_date."   ".$ddldb."   ".$wallet;exit;
	    if($wallet == "DMT")
	    {
    			...
    			act.host_id = ?
    			order by a.Id desc";
        		$rslt = $this->db->query($str_query,array($user_id,$from_date,$to_date,$host_id));
        		return $rslt;
	    ...
	    else
	    {
	        if($ddldb == "ARCHIVE")
		    {
    			...
    			dr.mobile_no as dr_mobile_no,
    			dr.usertype_name as dr_usertype_name
    			from symkgqcr_archive.tblewallet a 
    			left join tblusers act on a.user_id = act.user_id
    			left join symkgqcr_archive.tblpayment pay on a.payment_id = pay.payment_id
    			left join tblusers cr on pay.cr_user_id = cr.user_id
    			left join tblusers dr on pay.dr_user_id = dr.user_id
    			where 
    			a.user_id = ? and 
	...
	private function AccountLedger_getReport_rows($user_id,$from_date,$to_date,$ddldb,$wallet)
	{
	    if($wallet == "DMT")
	    {
	         if($ddldb == "ARCHIVE")
		    {
    			...
    			select 
    			count(a.Id) as total
    			from symkgqcr_archive.tblewallet2 a 
    			where 
    			a.user_id = ? and 
    			DATE(a.add_date) >= ? and 
    			DATE(a.add_date) <= ?
