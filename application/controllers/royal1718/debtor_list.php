...
class Debtor_list extends CI_Controller {
	...
	if($totalsums->num_rows() == 1)
	{
		...
		{
			$debtorrslt = $this->db->query("SELECT a.Id,a.Name,a.ref_name,a.add_date,a.mobile_no,a.Address,a.user_id,
			b.opening_outstanding,
			b.opening_date,
			(select Sum(received_cash)  from cashflow where cashflow.debtor_id = a.Id) as received_cash,
			(select IFNULL(Sum(sales.credit_amount),0)   from tblsales sales where sales.ew_user_id = a.user_id and description not like '%Flat Commission %' ) as totalcredit,
			(select IFNULL(Sum(sales_d.debit_amount),0)   from tblsales sales_d where sales_d.ew_user_id = a.user_id and description not like '%Flat Commission %' ) as totaldebit,
			(select IFNULL(Sum(bank.Deposit),0)   from tblBankLedger bank where bank.debtor_id = a.Id ) as totaldeposit
			
			FROM tbldebtors a 
			left join drcropening b on a.Id = b.crdruser_id and b.user_type = 'DEBTOR'
			...
			$this->load->view("Admin/debtor_list_view",$this->view_data);
		}
