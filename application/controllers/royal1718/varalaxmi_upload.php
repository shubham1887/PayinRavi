...
class Varalaxmi_upload extends CI_Controller {
		 	...
		 	if(isset($_POST["btnSubmit"])) 
			{
				...
				if(isset($_FILES["file"])) 
				{
					...
					else 
					{
						...
						$storagename = "UserImages/".$this->common->getDate()."/laxmi.xls";
						...
						{	
							if(count($data->sheets[$i][cells])>0) // checking sheet not empty
							{
								...
								{ 
									...
									$Name = str_replace("?","",mb_convert_encoding($data->sheets[$i][cells][$j][3], 'UTF-8', 'UTF-8'));
									$number = str_replace("?","",mb_convert_encoding($data->sheets[$i][cells][$j][4], 'UTF-8', 'UTF-8'));
									$amount = str_replace("?","",mb_convert_encoding($data->sheets[$i][cells][$j][7], 'UTF-8', 'UTF-8'));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////						
				...
				$this->load->view('Admin/varalaxmi_upload_view',$this->view_data);		
			}
			...
			//$this->db->query("update tblBankLedger set creditor_id = ?,Remark = ? where Id = ?",array($did,$Remark,$Id));
						...
						$accountinfo = $this->db->query("select * from tblbankdetail where Id = 5");
					//	print_r($accountinfo->result());exit;
						if($accountinfo->num_rows() == 1)
						{
							$bank_account_id = $accountinfo->row(0)->Id;
							$bank_id = $accountinfo->row(0)->bank_id;
							...
							// echo $txnDate;exit;
 							...
 							//print_r($bankledgerinfo->num_rows());exit;
							if($bankledgerinfo->num_rows() == 0)
							{
								$ex_BranchCode = $Cheque_no;
								$ex_RefNo = $Currency;
								$ex_ValueDate = $txnDate;
								$this->db->query("insert into tblBankLedger(user_id,hostname,add_date,ipaddress,transaction_date,Withdrawal,Deposit,Balance,Narration,bank_id,bank_account_id,branchcode,BankRefNo,sValueDate,creditor_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",array(1,"account.quickapp.in",$this->common->getDate(),$this->common->getRealIpAddr(),$txnDate,$Debit,$Credit,$Balance,$Description,$bank_id,$bank_account_id,$ex_BranchCode,$ex_RefNo,$ex_ValueDate,2));
							//	echo "end";
				...
				redirect(base_url()."Admin/varalaxmi_upload");
			}
			else
			{
					$this->view_data['msg'] = "";
