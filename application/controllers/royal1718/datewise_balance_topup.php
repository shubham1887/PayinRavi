...
class Datewise_balance_topup extends CI_Controller {
	...
	public function index()
	{
		...
		else if($this->input->post("btnSearch"))
		{
			...
			$user_id = $ddldebtor;
			$rslt_bank_ledger = $this->db->query("SELECT a.Id,a.EwId,a.payment_id,a.remark,a.description,a.Ew_add_date,IFNULL(Sum(a.credit_amount),0) as credit_amount,IFNULL(Sum(a.debit_amount),0) as debit_amount,a.balance,a.ew_user_id,
	...
	where 
	d.Id = ? and 
	Date(a.Ew_add_date) >= ? and 
	Date(a.Ew_add_date) <= ? and 
	a.remark NOT LIKE  'Flat Commission%'
	group by Date(Ew_add_date)
	order by a.payment_id",array($user_id,$txtFromDate,$txtToDate));
			...
			$this->load->view("Admin/datewise_balance_topup_view",$this->view_data);
