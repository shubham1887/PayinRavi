...
class Pnbuploadfile extends CI_Controller {
		 	...
		 	if(isset($_POST["btnSubmit"])) 
			{
				...
				if(isset($_FILES["file"])) 
				{
					...
					else 
					{
						...
						$storagename = "UserImages/".$this->common->getDate()."/pnb.xls";
							...
							if(count($data->sheets[$i][cells])>0) // checking sheet not empty
							{
								...
								/*	$temparray = array(
									...
									*/
									...
									$In_txnDate = str_replace("?","",mb_convert_encoding($data->sheets[$i][cells][$j][2], 'UTF-8', 'UTF-8'));
									$In_txnDate = str_replace("/","-",$In_txnDate);
									...
									$In_Credit = str_replace("?","",mb_convert_encoding($data->sheets[$i][cells][$j][8], 'UTF-8', 'UTF-8'));
									$In_Balance = str_replace("?","",mb_convert_encoding($data->sheets[$i][cells][$j][9], 'UTF-8', 'UTF-8'));
									$In_Desc = str_replace("?","",mb_convert_encoding($data->sheets[$i][cells][$j][10], 'UTF-8', 'UTF-8'));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////						
				...
				$this->load->view('Admin/pnbuploadfile_view',$this->view_data);		
			}
			else if($this->input->post("hidaction") == "INSERTDATA")
			{
				...
				foreach($json_arr as $r)
				{
					...
					$txnDate = $r->txnDate;
					$Desc = $r->Desc;
					$RefCheque = $r->RefCheque;
					$Debit = $r->Debit;
					$Credit = $r->Credit;
					$Balance = $r->Balance;
					
					if(trim($Desc) != "Narration")
					{
						$accountinfo = $this->db->query("select * from tblbankdetail where Id = 1");
						if($accountinfo->num_rows() == 1)
						{
							$bank_account_id = $accountinfo->row(0)->Id;
							$bank_id = $accountinfo->row(0)->bank_id;
							...
							$bankledgerinfo = $this->db->query("select * from tblBankLedger where transaction_date = ? and Withdrawal = ? and Deposit = ? and Balance = ? and Narration = ? and bank_id = ? and bank_account_id = ?",array($txnDate,$Debit,$Credit,$Balance,$Desc,$bank_id,$bank_account_id));
							if($bankledgerinfo->num_rows() == 0)
							{
								$this->db->query("insert into tblBankLedger(user_id,hostname,add_date,ipaddress,transaction_date,Withdrawal,Deposit,Balance,Narration,bank_id,bank_account_id) values(?,?,?,?,?,?,?,?,?,?,?)",array(1,"account.quickapp.in",$this->common->getDate(),$this->common->getRealIpAddr(),$txnDate,$Debit,$Credit,$Balance,$Desc,$bank_id,$bank_account_id));
							}
						}
					}
					
				}
				redirect(base_url()."Admin/pnbuploadfile");
			}
			else
			{
					$this->view_data['msg'] = "";
