...
class LoadReport extends CI_Controller {
	...
	//	echo $start_row;exit;
		...
		$this->load->view('Retailer/LoadReport_view',$this->view_data);			
	}
...
/*
		...
		*/
		$str_query ="
				...
				{
					$totalPurchase = 0;
					$rsltsummary = $this->db->query("select IFNULL(Sum(a.credit_amount),0) as totalPurchase from tblewallet a where a.user_id = ? and a.transaction_type = 'PAYMENT' and Date(a.add_date) BETWEEN ? and ?",array($this->session->userdata("AgentId"),$from,$to));
