...
$this->output->set_header("Pragma: no-cache"); 
			...
			else
			{
				...
				if(trim($user) == 'Agent')
				{
					$this->view_data['userdata'] = $this->db->query("
						...
						where a.user_id = ?
						",array($this->session->userdata('AgentId')));

					

					$doc_array = array();
					$doc_rslt = $this->db->query("
						SELECT user_id,doc_type,image_path,status FROM mpay_documents.documents where user_id = ? and status = 'APPROVED' ",array($this->session->userdata('AgentId')));
					if($doc_rslt->num_rows() > 0)
					{
						foreach($doc_rslt->result() as $rwdoc)
						{
							$doc_array[$rwdoc->doc_type] = 	str_replace(":","_",$rwdoc->image_path);
						}
					...
					$this->load->view('Retailer/UserProfile_view',$this->view_data);			
				}
				else
				{redirect(base_url().'login');}																								
			}
	...
	public function UserProfileUpload()
	{
		if (is_uploaded_file($_FILES['profileImage']['tmp_name'])) 
	    {
            ...
            {
                redirect(base_url()."Retailer/UserProfile");
            ...
            else
            {
                ...
                $uploads_dir = "uploads/Profile/".$user_id.$this->common->getDate().$_FILES["profileImage"]["name"];
                $tmp_name = $_FILES['profileImage']['tmp_name'];
                $pic_name = $_FILES['profileImage']['name'];
                move_uploaded_file($tmp_name, $uploads_dir);
                $response .= "\nFile Uploaded Successfully to the server";
                $user_id = $this->session->userdata("AgentId");
                $this->db->query("update tblusers_info set profile_pic_path = ? where user_id = ?",array($uploads_dir ,$user_id));
