...
class Bbps extends CI_Controller {
  ...
  public function GetTransactiondetailsPay()
  {
...
*/
          if($company_info->row(0)->service_id == 3 and $company_info->row(0)->company_id == 44)
          {
  ...
  public function Dotransactions()
  {
    ...
    */
    ...
    if(isset($json_obj->billercode) and isset($json_obj->Amount) and isset($json_obj->custmobileno)  )
    {
        ...
        $payment_mode =  trim((string)$json_obj->payment);
      $billercode =  trim((string)$json_obj->billercode);
      $billnumber =  trim((string)$json_obj->billnumber);
      ...
      if($company_info->num_rows() == 1)
      {
          ...
          $resp = $this->BillPayment->recharge_transaction2($userinfo,$company_info->row(0)->company_id,$Amount,$billnumber,$custmobileno,$remark,$option1,$ref_id,$particulars,$option2,$option3,$done_by,$payment_mode);
		...
		{ 
			...
			if(trim($user) == 'Agent')
			{
				...
				{		
						...
						$this->load->view('Retailer/Bbps_view',$this->view_data);
				}
			} 
		}
	}
