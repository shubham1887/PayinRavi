...
class Recharges_Report extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
	...
	//print_r($this->input->post());exit;
		...
		else if($this->input->post('FromDate') and $this->input->post('ToDate'))
		{
			...
			$this->view_data['result_all'] = $this->db->query("SELECT a.recharge_id,a.add_date,b.company_name,a.mobile_no,a.amount,a.recharge_status,a.operator_id
                                                        FROM `tblrecharge` a
                                                        left join tblcompany b on a.company_id = b.company_id

where a.user_id = ? and Date(a.add_date) >= ? and Date(a.add_date) <= ? order by recharge_id desc",array($user_id,$from,$to));
			...
			$this->load->view('Retailer/Recharges_Report_view',$this->view_data);								
		}
		else 
		{ 						
...
where a.user_id = ? and Date(a.add_date) >= ? and Date(a.add_date) <= ? order by recharge_id desc",array($user_id,$from,$to));
	...
	private function getTotalDMRValues($from,$to)
	{
		...
		$rslt = $this->db->query("select IFNULL(Sum(amount),0) as total,recharge_status from tblrecharge where Date(add_date) >= ? and Date(add_date) <= ? and user_id = ? group by recharge_status",array($from,$to,$user_id));
		...
		foreach($rslt->result() as $rw)
		{
			...
			else if($rw->recharge_status  == "HOLD")
			{
				$total_hold += $rw->total;
							}
		}
