...
class LoadReport2 extends CI_Controller {
	...
	//	echo $start_row;exit;
		...
		$config['base_url'] = base_url()."Retailer/LoadReport2/pageview";
		...
		$this->load->view('Retailer/LoadReport2_view',$this->view_data);			
	}
...
/*
		...
		*/
		$str_query ="
				...
				{
					$totalPurchase = 0;
					$rsltsummary = $this->db->query("select IFNULL(Sum(a.credit_amount),0) as totalPurchase from tblewallet2 a where a.user_id = ? and a.transaction_type = 'PAYMENT' and Date(a.add_date) BETWEEN ? and ?",array($this->session->userdata("AgentId"),$from,$to));
					if($rsltsummary->num_rows() == 1)
					{
						$totalPurchase = round($rsltsummary->row(0)->totalPurchase,2);
					}
    ...
    $fileName = "Payment Load Report From ".$from." To  ".$to.".xls";
