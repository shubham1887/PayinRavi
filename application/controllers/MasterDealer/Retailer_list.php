...
class Retailer_list extends CI_Controller {
	...
	public function pageview()
	{
		...
		$userrows = $this->db->query("select count(user_id) as total from tblusers where usertype_name = 'Agent' and parentid IN (select user_id from tblusers where parentid = ?)",array($user_id));
		
		$total_row = $userrows->row(0)->total;
		$this->load->library('pagination');
		$config['base_url'] = base_url()."MasterDealer/retailer_list/pageview";
		...
		where 
		a.usertype_name = 'Agent'  and a.parentid IN (select user_id from tblusers where parentid = ?)
		...
		$this->load->view('MasterDealer/retailer_list_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			...
			{
			    $parent = $this->Common_methods->decrypt(trim($_GET["parentid"]));
			  
			    $result = $this->Search("PARENTID",$parent);	
	...
	public function Search($SearchBy,$SearchWord)
	{
		...
		left join tblusers p on a.parentid = p.user_id
		where 
		a.usertype_name = 'Agent' and a.parentid IN (select user_id from tblusers where parentid = ?) and
	...
	//	print_r($result->num_rows());exit;
		...
		{
			redirect(base_url()."MasterDealer/retailer_list?crypt=".$this->Common_methods->encrypt("MyData"));
		}				
	}
