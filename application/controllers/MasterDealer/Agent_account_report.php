...
$this->output->set_header("Pragma: no-cache"); 
	...
	private function AccountLedger_getReport($user_id,$from_date,$to_date)
	{
		...
		where 
		a.user_id = ? and 
		DATE(a.add_date) >= ? and DATE(a.add_date) <= ? and b.parentid = ? order by a.Id desc";
			$rslt = $this->db->query($str_query,array($user_id,$from_date,$to_date,$this->session->userdata("MdId")));
			return $rslt;
