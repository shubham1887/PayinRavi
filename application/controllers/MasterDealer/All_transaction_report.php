...
$this->output->set_header("Pragma: no-cache"); 
	...
	private function getrechargerowcount($from,$to,$user_id)
	{
		...
		{
			$countarr = $this->db->query("select IFNULL(count(recharge_id),0) as total from maharshi_archivdata.tblrecharge where Date(tblrecharge.add_date) >='$from' and Date(tblrecharge.add_date) <= '$to' and tblrecharge.MdId = ?  and if(?!='ALL',tblrecharge.user_id = ?,true) ",array($this->session->userdata("MdId"),$user_id,$user_id));
		
			return $countarr->row(0)->total;
		}
		else if($from_date > $archivdate and $to_date > $archivdate)
		{
			$countarr = $this->db->query("select IFNULL(count(recharge_id),0) as total from tblrecharge where Date(add_date) >='$from' and Date(add_date) <= '$to' and tblrecharge.MdId = ?  and if(?!='ALL',tblrecharge.user_id = ?,true) ",array($this->session->userdata("MdId"),$user_id,$user_id));
