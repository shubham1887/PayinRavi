...
class Debtors_report extends CI_Controller {
		...
		{ 
			if($this->input->post("ddluser") and $this->input->post("txtFrom") and $this->input->post("txtTo"))
			{
				...
				if($rsltpayee->num_rows() == 1)
				{
					...
					$rsltuserbalance = $this->db->query("select a.*,b.business_name,b.username from ACCOUNTING_DEBTORS_ACCOUNT a left join tblusers b on a.USER_ID = b.user_id  where a.USER_ID = ? order by a.Id ",array($ddluser));
					$this->view_data["data"] = $rsltuserbalance;
					$this->view_data["message"] = "";
					$this->load->view('Account/debtors_report_view',$this->view_data);	
