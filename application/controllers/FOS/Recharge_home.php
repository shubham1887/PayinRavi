...
class Recharge_home extends CI_Controller {
	...
	{		
		...
		$balance = $this->Common_methods->getAgentBalance($this->session->userdata("FOSId"));	
		...
		{ 
			...
			if(trim($user) == 'Distributor')
			{
				if($this->input->post("hidSubmitRecharge") == "Success")				
				{
					...
					$scheme_id = $this->session->userdata("DistSchemeId");
