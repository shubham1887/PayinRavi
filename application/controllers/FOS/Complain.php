...
class Complain extends CI_Controller {
	...
	public function pageview()
	{
		...
		$result = $this->db->query("select complain_id from tblcomplain where user_id = ?",array($this->session->userdata("FOSId")));
		$total_row = $result->num_rows();		
		$this->load->library('pagination');
		$config['base_url'] = base_url()."FOS/complain/pageview";
		$config['total_rows'] = $total_row;
		$config['per_page'] = $per_page; 
		$this->pagination->initialize($config); 
		$this->view_data['pagination'] = $this->pagination->create_links();
		$this->view_data['result_complain'] = $this->db->query("select * from tblcomplain where user_id = ? order by complain_id desc limit ?,?",array($this->session->userdata("FOSId"),$start_row,$per_page));
		$this->view_data['message'] =$this->msg;
		$this->view_data['cmpl_flag'] =0;
		$this->load->view('FOS/complain_view',$this->view_data);	
...
$this->output->set_header("Pragma: no-cache"); 
		...
		$this->Sms->complainsms($userinfo->row(0)->username,$userinfo->row(0)->businessname);			
				...
				redirect(base_url()."FOS/complain");				
			}
