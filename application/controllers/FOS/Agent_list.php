...
class Agent_list extends CI_Controller {
	...
	public function pageview()
	{
		...
		$userrows = $this->db->query("select count(user_id) as total from tblusers where usertype_name = 'Agent' and fos_id = ?",array($user_id));
		
		$total_row = $userrows->row(0)->total;
		$this->load->library('pagination');
		$config['base_url'] = base_url()."FOS/agent_list/pageview";
		...
		where 
		a.usertype_name = 'Agent'  and a.fos_id = ?
		...
		$this->load->view('FOS/agent_list_view',$this->view_data);		
	}
...
$this->output->set_header("Pragma: no-cache"); 
	...
	public function Search($SearchBy,$SearchWord)
	{
		...
		left join tblusers p on a.parentid = p.user_id
		where 
		a.usertype_name = 'Agent' and a.fos_id = ? and
		...
		left join tblusers p on a.parentid = p.user_id
		where 
		a.usertype_name = 'Agent' and and a.fos_id = ? and
		...
		{
			redirect(base_url()."FOS/agent_list?crypt=".$this->Common_methods->encrypt("MyData"));
		}				
	}
