...
public function index() 
{
	if($this->input->post('btnSearch'))
	{
		...
		$this->load->model('Agent_report_model');
					...
					from tblrecharge where Date(add_date) >=? and Date(add_date) <=?  and tblrecharge.user_id IN(select user_id from tblusers where fos_id = ?) group by company_id";
		...
		$this->load->view('FOS/operator_report_view',$this->view_data);								
	}
	else 
	{ 						
