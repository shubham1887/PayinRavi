...
class Aeps_ledger extends CI_Controller {
...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$rsltcredit = $this->db->query("select IFNULL(Sum(credit_amount),0) as total from aeps_tblewallet where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ?",array($user_id,$from_date,$to_date));
				$rsltdebit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from aeps_tblewallet where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($user_id,$from_date,$to_date));
				...
				$this->load->view('API/Aeps_ledger_view',$this->view_data);		
			}					
			else
			{
	...
	private function AccountLedger_getReport($user_id,$from_date,$to_date,$ddlpaymenttype,$ddldb)
	{
	    ...
	    if($ddldb == "ARCHIVE")
	    {
	        ...
	        from masterpa_archive.aeps_tblewallet  a
