    ...
    left join tblusers b on a.user_id = b.user_id
     ...
     order by Id desc limit 50",array($user_id,$from,$to,$ddlstatus,$ddlstatus,$txtRemitter,$txtRemitter,$txtAccNo,$txtAccNo));    
			...
			else if($this->input->post('hidrecid') and $this->input->post('hidmsg'))
    		{
    				...
    				$user_id = $this->session->userdata('ApiId');	
    				$recharge_info = $this->db->query("select * from mt3_transfer where Id = ? and Status = 'PENDING'",array($hidrecid));
    				if($recharge_info->num_rows() == 1)
    				{
    					redirect(base_url()."API/dmr_report");
    				}
    				else
    				{
    					$recinfo = $this->db->query("select * from tblcomplain_dmt where complain_status = 'Pending' and dmr_id = ?",array($hidrecid));
