...
FROM `mt3_transfer` a
 where Date(a.add_date) = ? and a.user_id = ? order by Id desc limit 10",array($this->common->getMySqlDate(),$user_id));
		...
		public function getBalance()
		{
			...
			echo round($this->Common_methods->getAgentBalance($this->session->userdata("ApiId")))."#".round($this->Ew2->getAgentBalance($this->session->userdata("ApiId")))."#".round($this->Ewallet_aeps->getAgentBalance($this->session->userdata("ApiId")),3);
		}
	}