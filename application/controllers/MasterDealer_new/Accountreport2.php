...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 	
			if($this->input->post('btnSearch') == "Search")
			{
				...
				$this->view_data['result_mdealer'] = $this->AccountLedger_getReport($this->session->userdata("MdId"),$from_date,$to_date,$ddlpaymenttype);
				$this->view_data['message'] =$this->msg;
				
				$rsltdebit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ?",array($this->session->userdata("MdId"),$from_date,$to_date));
				...
				$this->load->view('MasterDealer_new/accountreport2_view',$this->view_data);		
			}					
			else
			{
				...
				$this->view_data['pagination'] = NULL;
				$this->view_data['result_mdealer'] = $this->AccountLedger_getReport($this->session->userdata("MdId"),$from_date,$to_date,"ALL");
				$this->view_data['message'] =$this->msg;
				$rsltcredit = $this->db->query("select IFNULL(Sum(credit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("MdId"),$from_date,$to_date));
				$rsltdebit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("MdId"),$from_date,$to_date));
	...
	private function gettotalcredit($user_id,$from_date,$to_date)
	{
		...
		if($from_date <= $archivdate and $to_date <= $archivdate)
		{
			...
			$rsltcredit = $this->db->query("select IFNULL(Sum(credit_amount),0) as total from gofullrc_archiv.tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("MdId"),$from_date,$to_date));
	...
	private function gettotaldebit($user_id,$from_date,$to_date)
	{
		...
		if($from_date <= $archivdate and $to_date <= $archivdate)
		{
			...
			$rsltcredit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from gofullrc_archiv.tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("MdId"),$from_date,$to_date));
			return $rsltcredit->row(0)->total;	
		...
		{
			$rsltcredit = $this->db->query("select IFNULL(Sum(debit_amount),0) as total from tblewallet2 where user_id = ? and Date(add_date) >= ? and Date(add_date) <= ? ",array($this->session->userdata("MdId"),$from_date,$to_date));
			return $rsltcredit->row(0)->total;
		}
		...
		if(isset($_GET["from"]) and isset($_GET["to"]))
		{
			...
			$data = array();
				...
				left join tblusers dr on pay.dr_user_id = dr.user_id
				 ...
				 a.user_id = ?
				 order by Id";
		$rslt = $this->db->query($str_query,array($from,$to,$this->session->userdata("MdId")));
		$i = 0;
