			...
			else
			{
				...
				if(trim($user) == 'MasterDealer')
				{
					...
					$doc_rslt = $this->db->query("
						SELECT user_id,doc_type,image_path,status FROM mpay_documents.documents where user_id = ? and status = 'APPROVED' ",array($this->session->userdata('MdId')));
					...
					$this->load->view('MasterDealer_new/UserProfile_view',$this->view_data);			
				}
				else
				{redirect(base_url().'login');}																								
			}
