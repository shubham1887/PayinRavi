...
$this->output->set_header("Pragma: no-cache"); 
		...
		{ 
			...
			if(isset($_POST["ddlsubject"]) and isset($_POST["message"]))
			{
			    ...
			    {
			    	$recharge_info = $this->db->query("select recharge_id,amount,mobile_no,add_date,recharge_status,user_id from tblrecharge where recharge_id = ? and user_id = ? ",array($tranid,intval($this->session->userdata("MdId"))));	
			    	//print_r($recharge_info->num_rows());exit;
			    	if($recharge_info->num_rows() == 1)
			    	{
			    		$rsltcomplain = $this->db->query("select * from tblcomplain where recharge_id = ? and user_id = ?",array($tranid,intval($this->session->userdata("MdId"))));
			    		if($rsltcomplain->num_rows() == 0)
			    		{
			    			$this->db->query("insert into tblcomplain(list_id,transacation_id,recharge_id,user_id,complain_date,complain_status,message,complain_type,host_id)
			    				values(?,?,?,?,?,?,?,?,?)",
			    				array(0,0,$tranid,$this->session->userdata("MdId"),$this->common->getDate(),"Pending",$message,$ddlsubject,1));
	    					...
	    					$this->load->view('MasterDealer_new/Raisecomplain_view',$this->view_data);
			    		}
			    		else
			    		{
			    ...
			    {
		    		$rsltcomplain = $this->db->query("select * from tblcomplain where message = ? and user_id = ?",array($message,intval($this->session->userdata("MdId"))));
		    		if($rsltcomplain->num_rows() == 0)
		    		{
		    			$this->db->query("insert into tblcomplain(list_id,transacation_id,recharge_id,user_id,complain_date,complain_status,message,complain_type,host_id)
		    				values(?,?,?,?,?,?,?,?,?)",
