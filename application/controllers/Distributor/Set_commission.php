...
class Set_commission extends CI_Controller {
	...
	function getCommission()
	{
	    return "";
			...
			if($company_info->num_rows() == 1)
			{
				...
				$dist_commissioninfo = $this->getCommissionInfo($company_id,$this->session->userdata("DistId"),$this->session->userdata("DistSchemeId"));
				...
				$userinfo = $this->db->query("
					select 
						...
						where a.parentid = ? and a.usertype_name = 'Agent'
	...
	function ChangeCommission()
	{
		...
		$userinfo = $this->db->query("select * from tblusers where user_id = ? and parentid = ?",array($user_id,$this->session->userdata("DistId")));
				...
				if($userinfo->row(0)->usertype_name == "Agent")
				{
					...
					if($com <= $parent_commission_per)
					{
						if($com <= $max_com_limit and $com >= $min_com_limit)
						{
							...
							$fosuserinfo = $this->db->query("select * from tblusers where user_id = ? and parentid = ?",array($fos_id,$this->session->userdata("DistId")));
