...
class All_transaction_report extends CI_Controller {
	...
	$total_row = $this->getrechargerowcount($from,$to,$user);
		...
		$config['base_url'] = base_url()."Distributor/all_transaction_report/commonfunction";
...
$this->view_data['totalfRecahrge'] =$totalfailure;
		...
		$this->load->view('Distributor/all_transaction_report_view',$this->view_data);			
	...
	private function get_recharge_all($from_date,$to_date,$user_id)
	{
		...
		if($from_date <= $archivdate and $to_date <= $archivdate)
		{
			$str_query ="select update_time, DComm,tblrecharge.recharge_id,tblrecharge.mobile_no,tblrecharge.amount,tblrecharge.recharge_status,tblrecharge.add_date,tblrecharge.operator_id,tblrecharge.commission_amount,tblrecharge.commission_per,tblrecharge.recharge_by,company_name,businessname,username,company_name from maharshi_archivdata.tblrecharge,tblcompany,tblusers where 
				tblusers.user_id = tblrecharge.user_id and (tblusers.usertype_name = 'Agent' or tblusers.usertype_name = 'Distributor' or tblusers.usertype_name = 'APIUSER') and
...
private function get_recharge($from_date,$to_date,$user_id,$startrow,$perpage)
	...
	private function getrechargerowcount($from,$to,$user_id)
	{
		...
		if($from_date <= $archivdate and $to_date <= $archivdate)
		{
			$countarr = $this->db->query("select IFNULL(count(recharge_id),0) as total from maharshi_archivdata.tblrecharge where Date(tblrecharge.add_date) >='$from' and Date(tblrecharge.add_date) <= '$to' and tblrecharge.user_id = ?  and if(?!='ALL',tblrecharge.user_id = ?,true) ",array($this->session->userdata("DistId"),$user_id,$user_id));
		
			return $countarr->row(0)->total;
		}
		else if($from_date > $archivdate and $to_date > $archivdate)
		{
			$countarr = $this->db->query("select IFNULL(count(recharge_id),0) as total from tblrecharge where Date(add_date) >='$from' and Date(add_date) <= '$to' and tblrecharge.user_id = ?  and if(?!='ALL',tblrecharge.user_id = ?,true) ",array($this->session->userdata("DistId"),$user_id,$user_id));
