...
$this->output->set_header("Pragma: no-cache"); 
    ...
    order by Id desc",array($user_id,$from,$to,$txtUserId,$txtUserId,$txtUserId));
    ...
    FROM `mt3_transfer` a where a.user_id = ? and Date(a.add_date) >= ? and Date(a.add_date) <= ? order by Id desc",array($user_id,$from,$to));
...
left join tblusers parent on b.parentid = parent.user_id
 where Date(a.add_date) >= ? and Date(a.add_date) <= ?  and a.DId = ?
 
 order by Id desc",array($from,$to,$this->session->userdata("DistId")));
		$i = 0;
