...
rgapp.controller('MAINRGBS', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.dobindopnamebycate = function (item) {
        ...
        if (item.Categoryid) {
            $http({
                url: '/Retailer/Bbps/SubCategories?category=' + item.Categoryid,
                method: 'POST'
            }).then(function successCallback(response) {
                if (response.data.ResponseStatus === '1') {
    ...
    $scope.dobindvalidateparam = function (item) {
        resetmethod();
        if (item) {
            $scope.OperatorName = item.Billername;
            $http({
                url: '/Retailer/Bbps/GetBillerparams?billerid=' + item.Billerid,
                method: 'POST'
            }).then(function successCallback(response) {
                if (response.data.ResponseStatus === '1') {
                    $scope.Billerparamdata = response.data.Data; 
                    var cnt = 0;
                    ...
                    if ($scope.Biller_Name.Billercode === 'BSLL') 
                        { $scope.pptypenumbernumverview = false; } 
                    if ($scope.Biller_Name.Validateallow === true) 
                        { 
                            $scope.btnpaymentprocess = true; 
                            $scope.btnfeachindvalidateinfo = false;
                            $scope.payable_amount = true; 
                         } 
                         else 
                            { 
                                $scope.btnpaymentprocess = false; 
                                $scope.btnfeachindvalidateinfo = true; 
                                $scope.payable_amount = false; 
                            }
                } else {
                    swal('', response.data.Remarks, 'error');
    ...
    function doshowprintforbbps(itemvalue) {
        $.post("/Retailer/bbps//GetTransactiondetailsPay", { TranId: itemvalue }, function (item) {
            showprtreciept(item);
    ...
    $scope.doforfeachingbalfor = function () {
        ...
        if ($scope.Biller_Name && $scope.MainNumber && $scope.Retailer_MobileNumber && $scope.Payment_Mode && $scope.Pay_TPIN) {
            ...
            $http({
                url: '/Retailer/Bbps/Validatebillingdetails',
                method: 'POST',
                ...
                if (response.data.StatusCode === 1) {
                    ...
                    $scope.shobillerimage = '../mpayfiles/' + $scope.Biller_Name.Imageurl;
        ...
        if ($scope.Biller_Name && $scope.MainNumber && $scope.Retailer_MobileNumber && $scope.Payment_Mode && $scope.Pay_Amount > 0 && $scope.Pay_TPIN) {
            ...
            $http({
                ...
                if (jsonobj.StatusCode === 1 | jsonobj.StatusCode === "1") {
                    ...
                    jQuery('#btnsuccessPrint').attr('onclick', 'billprint(' + jsonobj.Data.Rechargeid + ')');
                    resetmethod();
                } else {
